...
#define ALIAS(alias, name)                  \
    if (ass_strcasecmp(tname, #alias) == 0) \
    {                                       \
        tname = #name;                      \
    }
...
void ass_process_force_style(ASS_Track *track)
{
    ...
    for (fs = list; *fs; ++fs)
    {
        ...
        *eq = '\0';
        ...
        {
            if (style == NULL || ass_strcasecmp(track->styles[sid].Name, style) == 0)
            {
...
{
    if (!(track->parser_priv->header_flags & SINFO_SCALEDBORDER) && !format_line_compare(fmt, std))
    {
...
static bool detect_legacy_conv_subs(ASS_Track *track)
{
    ...
    if (track->parser_priv->header_flags != (SINFO_SCRIPTTYPE | SINFO_PLAYRESX | SINFO_PLAYRESY | GENBY_FFMPEG))
        return false;

    // Legacy ffmpeg only ever has one style
    // Check 2 not 1 because libass also adds a def style
    if (track->n_styles != 2 || strncmp(track->styles[1].Name, "Default", 7))
        return false;
                       ...
                       long long timecode, long long duration)
{
    ...
    memcpy(str, data, size);
    str[size] = '\0';
    ass_msg(track->library, MSGL_V, "Event at %lld, +%lld: %s",
                        ...
                        const char *codepage)
{
...

    {
        ...
        while (1)
        {
            ...
            { // clear the conversion state and leave
                clear = 1;
