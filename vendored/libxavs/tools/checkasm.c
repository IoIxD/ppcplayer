...
print_bench(void)
{
  ...
  for (i = 0; i < nfuncs; i++)
    for (j = 0; j < MAX_CPUS && (!j || benchs[i].vers[j].cpu); j++)
    {
      ...
      if (k < j)
        continue;
      printf("%s_%s%s: %lld\n", benchs[i].name, b->cpu & XAVS_CPU_SSE4 ? "sse4" : b->cpu & XAVS_CPU_SHUFFLE_IS_FAST ? "fastshuffle"
                                                                              : b->cpu & XAVS_CPU_SSSE3             ? "ssse3"
                                                                              : b->cpu & XAVS_CPU_SSE3              ? "sse3"
                                                                                                                    :
                                                                              ...
                                                                              : b->cpu & XAVS_CPU_ALTIVEC                                                                                           ? "altivec"
                                                                                                                                                                                                    : "c",
             b->cpu & XAVS_CPU_CACHELINE_32 ? "_c32" : b->cpu & XAVS_CPU_CACHELINE_64 ? "_c64"
                                                   : b->cpu & XAVS_CPU_SSE_MISALIGN   ? "_misalign"
                                                   : b->cpu & XAVS_CPU_LZCNT          ? "_lzcnt"
                                                                                      : "",
...
#define INTEGRAL_INIT(name, size, ...)                                                                                          \
  ...
  {                                                                                                                             \
    ...
    if (memcmp(buf3, buf4, (stride - 8) * 2) || (size > 9 && memcmp(buf3 + 18 * stride, buf4 + 18 * stride, (stride - 8) * 2))) \
      ok = 0;                                                                                                                   \
