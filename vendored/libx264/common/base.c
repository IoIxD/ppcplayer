...
#define OPT2(STR0, STR1) else if (!strcmp(name, STR0) || !strcmp(name, STR1))
    ...
    OPT("asm")
    {
        p->cpu = X264_ISDIGIT(value[0]) ? (uint32_t)atoi(value) : !strcasecmp(value, "auto") || atobool(value) ? x264_cpu_detect()
                                                                                                               : 0;
        if (b_error)
        {
    ...
    OPT("mastering-display")
    {
        ...
        {
            b_error |= sscanf(value, "G(%d,%d)B(%d,%d)R(%d,%d)WP(%d,%d)L(%lld,%lld)",
    ...
    OPT("fps")
    {
        int64_t i_fps_num;
        int64_t i_fps_den;
        if (sscanf(value, "%lld/%lld", &i_fps_num, &i_fps_den) == 2)
        {
...
char *x264_param2string(x264_param_t *p, int b_res)
{
    ...
    s += sprintf(s, " interlaced=%s", p->b_interlaced ? p->b_tff ? "tff" : "bff" : p->b_fake_interlaced ? "fake"
                                                                                                        : "0");
    ...
    s += sprintf(s, " rc=%s mbtree=%d", p->rc.i_rc_method == X264_RC_ABR   ? (p->rc.b_stat_read ? "2pass" : p->rc.i_vbv_max_bitrate == p->rc.i_bitrate ? "cbr"
                                                                                                                                                       : "abr")
                                        : p->rc.i_rc_method == X264_RC_CRF ? "crf"
                                                                           : "cqp",
                 p->rc.b_mb_tree);
