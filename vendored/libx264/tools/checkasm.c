    ...
    for (int i = 0; i < nfuncs; i++)
        for (int j = 0; j < MAX_CPUS && (!j || benchs[i].vers[j].cpu); j++)
        {
            ...
            printf("%s_%s%s: %lld\n", benchs[i].name,
                   ...
                   b->cpu & X264_CPU_AVX512 ? "avx512" : b->cpu & X264_CPU_AVX2 ? "avx2"
...
#elif ARCH_ARM
                   b->cpu & X264_CPU_NEON ? "neon" : b->cpu & X264_CPU_ARMV6 ? "armv6"
                                                                             :
#elif ARCH_AARCH64
                   b->cpu & X264_CPU_SVE2 ? "sve2" : b->cpu & X264_CPU_SVE ? "sve"
...
#elif ARCH_LOONGARCH
                   b->cpu & X264_CPU_LASX ? "lasx" : b->cpu & X264_CPU_LSX ? "lsx"
                                                                           :
#endif
                                                                             "c",
#if ARCH_X86 || ARCH_X86_64
                   b->cpu & X264_CPU_CACHELINE_32 ? "_c32" : b->cpu & X264_CPU_SLOW_ATOM && b->cpu & X264_CPU_CACHELINE_64                ? "_c64_atom"
...
#define TEST_INTRA8_X9(name, cmp)                                                                                               \
    ...
    if (pixel_asm.ssd_nv12_core != pixel_ref.ssd_nv12_core)
    {
        ...
        for (int w = 8; w <= 360; w += 8)
        {
            ...
            {
                ok = 0;
                fprintf(stderr, "ssd_nv12: %ulld,%ulld != %ulld,%ulld\n",
...
#define INTEGRAL_INIT(name, size, offset, cmp_len, ...)                                                                                                  \
    ...
    {                                                                                                                                                    \
        ...
        if (memcmp(buf3 + 2 * offset, buf4 + 2 * offset, cmp_len * 2) || (size > 9 && memcmp(buf3 + 18 * stride, buf4 + 18 * stride, (stride - 8) * 2))) \
            ok = 0;                                                                                                                                      \
...
#define TEST_QUANT(qname, block, type, w, maxj)                                                                                                               \
    ...
    {                                                                                                                                                         \
        ...
        {                                                                                                                                                     \
            ...
            {                                                                                                                                                 \
                INIT_QUANT##type(j, w *w) int result_c = call_c1(qf_c.qname, (void *)dct1, h->quant##type##_mf[block][qp], h->quant##type##_bias[block][qp]); \
    ...
    if (ip_a.predict_8x8_filter != ip_ref.predict_8x8_filter)
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            if (!(neighbor & MB_TOPLEFT))
                edge[15] = edge2[15] = 0;
            if (memcmp(edge + 7, edge2 + 7, (i & MB_TOPRIGHT ? 26 : i & MB_TOP ? 17
                                                                               : 8) *
                                                SIZEOF_PIXEL))
            {
            ...
            {                                                                                                                             \
                ...
                {                                                                                                                         \
                    ...
                    {                                                                                                                     \
                        ...
                        if (!rd)                                                                                                          \
                            memcpy(bitstream[1], bitstream[0], 0x400);                                                                    \
                        ...
                        if (!rd)                                                                                                          \
                            ok |= !memcmp(bitstream[1], bitstream[0], 0x400) && !memcmp(&cb[1], &cb[0], offsetof(x264_cabac_t, p_start)); \
                        if (!ok)                                                                                                          \
                        {                                                                                                                 \
...
{
    return check_pixel(cpu_ref, cpu_new) + check_dct(cpu_ref, cpu_new) + check_mc(cpu_ref, cpu_new) + check_intra(cpu_ref, cpu_new) + check_deblock(cpu_ref, cpu_new) + check_quant(cpu_ref, cpu_new) + check_cabac(cpu_ref, cpu_new) + check_bitstream(cpu_ref, cpu_new);
}
