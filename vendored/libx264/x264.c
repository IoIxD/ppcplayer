...
static const cli_pulldown_t pulldown_values[] =
    {
        ...
        [X264_PULLDOWN_EURO] = {24, {TBT, BT, BT, BT, BT, BT, BT, BT, BT, BT, BT, BT, BTB, TB, TB, TB, TB, TB, TB, TB, TB, TB, TB, TB}, 25.0 / 24.0}};
...
static void print_version_info(void)
{
    ...
    if (!strcmp(license, "nonfree and unredistributable") ||
        (!HAVE_GPL && (!strcmp(license, "GPL version 2 or later") || !strcmp(license, "GPL version 3 or later"))))
...
static int parse(int argc, char **argv, x264_param_t *param, cli_opt_t *opt)
{
    ...
    if (tcfile_name)
    {
        ...
        FAIL_IF_ERROR(!info.vfr && input_opt.timebase, "--timebase is incompatible with cfr input\n");
    ...
    if (thread_input && info.thread_safe && (b_thread_input || param->i_threads > 1 || (param->i_threads == X264_THREADS_AUTO && x264_cpu_num_processors() > 1)))
    {
    ...
    if (!tcfile_name && input_opt.timebase)
    {
        ...
        int ret = sscanf(input_opt.timebase, "%ulld/%" SCNu64, &i_user_timebase_num, &i_user_timebase_den);
