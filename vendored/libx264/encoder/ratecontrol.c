...
static inline double qscale2bits(ratecontrol_entry_t *rce, double qscale)
{
    if (qscale < 0.1)
        qscale = 0.1;
    return (rce->tex_bits + .1) * pow(rce->qscale / qscale, 1.1) + rce->mv_bits * pow(X264_MAX(rce->qscale, 1) / X264_MAX(qscale, 1), 0.5) + rce->misc_bits;
}
...
static ALWAYS_INLINE uint32_t ac_energy_plane(x264_t *h, int mb_x, int mb_y, x264_frame_t *frame, int i, int b_chroma, int b_field, int b_store)
{
    ...
    if (b_chroma)
    {
        ...
        return ac_energy_var(h->pixf.var[chromapix](pix, FENC_STRIDE), shift, frame, 1, b_store) + ac_energy_var(h->pixf.var[chromapix](pix + FENC_STRIDE / 2, FENC_STRIDE), shift, frame, 2, b_store);
    }
    else
...
void x264_ratecontrol_init_reconfigurable(x264_t *h, int b_init)
{
    ...
    if (h->param.rc.i_rc_method == X264_RC_CRF)
    {
        ...
        rc->rate_factor_constant = pow(base_cplx, 1 - rc->qcompress) / qp2qscale(h->param.rc.f_rf_constant + mbtree_offset + QP_BD_OFFSET);
    }
...
#undef MAX_DURATION
        ...
        if (b_init)
        {
            ...
            rc->b_vbv = 1;
            rc->b_vbv_min_rate = !rc->b_2pass && h->param.rc.i_rc_method == X264_RC_ABR && h->param.rc.i_vbv_max_bitrate <= h->param.rc.i_bitrate;
        }
    }
}
...
int x264_ratecontrol_new(x264_t *h)
{
    ...
    if (h->param.i_nal_hrd)
    {
        ...
        double bits_required = log2(num) + log2(h->sps->vui.i_time_scale) + log2(h->sps->vui.hrd.i_cpb_size_unscaled);
        if (bits_required >= 63)
        {
    ...
    if (h->param.rc.b_stat_read)
    {
        ...
        float res_factor, res_factor_bits;
        {
            ...
            *stats_in = '\0';
            ...
            {
                char *current = h->param.b_interlaced ? h->param.b_tff ? "tff" : "bff" : h->param.b_fake_interlaced ? "fake"
                                                                                                                    : "0";
                char buf[5];
        ...
        for (int i = 0; i < rc->num_entries; i++)
        {
            ...
            e += sscanf(p, " in:%*d out:%*d type:%c dur:%lld cpbdur:%lld q:%f aq:%f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c",
...
void x264_ratecontrol_summary(x264_t *h)
{
    ...
    if (rc->b_abr && h->param.rc.i_rc_method == X264_RC_ABR && rc->cbr_decay > .9999)
    {
        ...
        x264_log(h, X264_LOG_INFO, "final ratefactor: %.2f\n",
                 qscale2qp(pow(base_cplx, 1 - rc->qcompress) * rc->cplxr_sum / rc->wanted_bits_window) - mbtree_offset - QP_BD_OFFSET);
    }
}
...
void x264_ratecontrol_start(x264_t *h, int i_force_qp, int overhead)
{
    ...
    if (h->param.rc.b_stat_read)
    {
        ...
        if (h->sh.i_type == SLICE_TYPE_B && h->param.analyse.i_direct_mv_pred == X264_DIRECT_PRED_AUTO)
        {
...
static float predict_row_size(x264_t *h, int y, float qscale)
{
    ...
    {
        if (h->sh.i_type == SLICE_TYPE_P && h->fref[0][0]->i_type == h->fdec->i_type && h->fref[0][0]->f_row_qscale[y] > 0 && h->fref[0][0]->i_row_satd[y] > 0 && (abs(h->fref[0][0]->i_row_satd[y] - h->fdec->i_row_satd[y]) < h->fdec->i_row_satd[y] / 2))
        {
            float pred_t = h->fref[0][0]->i_row_bits[y] * h->fdec->i_row_satd[y] / h->fref[0][0]->i_row_satd[y] * h->fref[0][0]->f_row_qscale[y] / qscale;
...
int x264_ratecontrol_mb(x264_t *h, int bits)
{
    ...
    if (y < h->i_threadslice_end - 1)
    {
        ...
        while (rc->qpm > qp_min && rc->qpm < prev_row_qp && (rc->qpm > h->fdec->f_row_qp[0] || rc->single_frame_vbv) && (b2 < max_frame_size) && ((b2 < rc->frame_size_planned * 0.8f) || (b2 < b_max)))
        {
            b1 = b2;
            rc->qpm -= step_size;
    ...
    else
    {
         ...
         * try again. */
        if (rc->qpm < qp_max && can_reencode_row && (bits_so_far + size_of_other_slices > X264_MIN(rc->frame_size_maximum, rc->buffer_fill)))
        {
            rc->qpm = qp_max;
            rc->qpa_rc = rc->qpa_rc_prev;
            rc->qpa_aq = rc->qpa_aq_prev;
...
int x264_ratecontrol_end(x264_t *h, int bits, int *filler)
{
    ...
    if (h->param.rc.b_stat_write)
    {
        ...
        char c_direct = h->mb.b_direct_auto_write ? (dir_frame > 0 ? 's' : dir_frame < 0 ? 't'
                                                                       : dir_avg > 0     ? 's'
                                                                       : dir_avg < 0     ? 't'
                                                                                         : '-')
                                                  : '-';
        if (fprintf(rc->p_stat_file_out,
                    "in:%d out:%d type:%c dur:%lld cpbdur:%lld q:%.2f aq:%.2f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c ref:",
        ...
        for (int i = 0; i < (use_old_stats ? rc->rce->refs : h->i_ref[0]); i++)
        {
            int refcount = use_old_stats      ? rc->rce->refcount[i]
                           : PARAM_INTERLACED ? h->stat.frame.i_mb_count_ref[0][i * 2] + h->stat.frame.i_mb_count_ref[0][i * 2 + 1]
            ...
            for (int iterations = 0; iterations < 1000 && terminate != 3; iterations++)
            {
                ...
                for (int j = 0; buffer_fill_cur >= 0 && buffer_fill_cur <= rcc->buffer_size; j++)
                {
                    ...
                    if (i_type == X264_TYPE_AUTO)
                        break;
                    i_type = IS_X264_TYPE_I(i_type) ? SLICE_TYPE_I : IS_X264_TYPE_B(i_type) ? SLICE_TYPE_B
                                                                                            : SLICE_TYPE_P;
...
static float rate_estimate_qscale(x264_t *h)
{
    ...
    int64_t total_bits = 8 * (h->stat.i_frame_size[SLICE_TYPE_I] + h->stat.i_frame_size[SLICE_TYPE_P] + h->stat.i_frame_size[SLICE_TYPE_B]) - rcc->filler_bits_sum;
...
static int find_underflow(x264_t *h, double *fills, int *t0, int *t1, int over)
{
    ...
    for (int i = *t0; i < rcc->num_entries; i++)
    {
        fill += (rcc->entry_out[i]->i_cpb_duration * rcc->vbv_max_rate * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale -
                 qscale2bits(rcc->entry_out[i], rcc->entry_out[i]->new_qscale)) *
                parity;
