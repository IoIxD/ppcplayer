...
static void report_new_stream(Demuxer *d, const AVPacket *pkt)
{
    ...
    av_log(d, AV_LOG_WARNING,
           "New %s stream with index %d at pos:%lld and DTS:%ss\n",
        ...
        if (fmt_is_discont)
        {
                ...
                pkt_dts + AV_TIME_BASE / 10 < ds->dts)
            {
                ...
                av_log(ist, AV_LOG_WARNING,
                       ...
                       "(stream id=%d): %lld, new offset= %lld\n",
...
static int demux_bsf_flush(Demuxer *d, DemuxThreadContext *dt)
{
    ...
    for (unsigned i = 0; i < f->nb_streams; i++)
    {
        ...
        ret = (ret == AVERROR_EOF) ? 0 : (ret < 0) ? ret
                                                   : AVERROR_BUG;
        if (ret < 0)
        {
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st, AVDictionary **opts_used)
{
    ...
    {
        const AVOption opts[] = {
            {"apply_cropping", NULL, 0, AV_OPT_TYPE_INT, {.i64 = CROP_ALL}, CROP_DISABLED, CROP_CONTAINER, AV_OPT_FLAG_DECODING_PARAM, .unit = "apply_cropping"},
