                     ...
                     uint64_t frame_num)
{
    ...
    for (size_t i = 0; i < es->nb_components; i++)
    {
        ...
        case ENC_STATS_TIMEBASE_IN:
            ...
            continue;
        case ENC_STATS_PTS:
            avio_printf(io, "%lld", pts);
            continue;
        case ENC_STATS_PTS_IN:
            avio_printf(io, "%lld", ptsi);
            continue;
...
static int frame_encode(OutputStream *ost, AVFrame *frame, AVPacket *pkt)
{
    ...
    {
        const AVSubtitle *subtitle = frame && frame->buf[0] ? (AVSubtitle *)frame->buf[0]->data : NULL;

        // no flushing for subtitles
        return subtitle && subtitle->num_rects ? do_subtitle_out(of, ost, subtitle, pkt) : 0;
    }
