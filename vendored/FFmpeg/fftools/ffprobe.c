...
static struct section sections[] = {
    ...
    [SECTION_ID_ROOT] = {SECTION_ID_ROOT, "root", SECTION_FLAG_IS_WRAPPER, {SECTION_ID_CHAPTERS, SECTION_ID_FORMAT, SECTION_ID_FRAMES, SECTION_ID_PROGRAMS, SECTION_ID_STREAM_GROUPS, SECTION_ID_STREAMS, SECTION_ID_PACKETS, SECTION_ID_ERROR, SECTION_ID_PROGRAM_VERSION, SECTION_ID_LIBRARY_VERSIONS, SECTION_ID_PIXEL_FORMATS, -1}},
                       ...
                       AVFormatContext *fmt_ctx)
{
    ...
    if (fd && fd->pkt_pos != -1)
        print_fmt("pkt_pos", "%lld", fd->pkt_pos);
    else
...
static void log_read_interval(const ReadInterval *interval, void *log_ctx, int log_level)
{
    ...
    if (interval->has_end)
    {
        ...
        if (interval->duration_frames)
            av_log(log_ctx, log_level, "#%lld", interval->end);
        else
                                 ...
                                 const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    if (interval->has_start)
    {
        ...
        {
            av_log(NULL, AV_LOG_ERROR, "Could not seek to position %lld: %s\n",
...
static int show_stream(WriterContext *w, AVFormatContext *fmt_ctx, int stream_idx, InputStream *ist, int container)
{
    ...
    if (stream->nb_frames)
        print_fmt("nb_frames", "%lld", stream->nb_frames);
    else
...
static const OptionDef real_options[] = {
    CMDUTILS_COMMON_OPTIONS{"f", OPT_TYPE_FUNC, OPT_FUNC_ARG, {.func_arg = opt_format}, "force format", "format"},
    ...
    {"byte_binary_prefix", OPT_TYPE_BOOL, 0, {&use_byte_value_binary_prefix}, "use binary prefixes for byte units"},
    {"sexagesimal", OPT_TYPE_BOOL, 0, {&use_value_sexagesimal_format}, "use sexagesimal format HOURS:MM:SS.MICROSECONDS for time units"},
    {"pretty", OPT_TYPE_FUNC, 0, {.func_arg = opt_pretty}, "prettify the format of displayed values, make it more human readable"},
    {"output_format", OPT_TYPE_STRING, 0, {&output_format}, "set the output printing format (available formats are: default, compact, csv, flat, ini, json, xml)", "format"},
    ...
    {"show_entries", OPT_TYPE_FUNC, OPT_FUNC_ARG, {.func_arg = opt_show_entries}, "show a set of specified entries", "entry_list"},
#if HAVE_THREADS
