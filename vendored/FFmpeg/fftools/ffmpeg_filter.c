                     ...
                     const OutputFilterOptions *opts)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        ofp->fps.framerate_supported = ost->force_fps || !opts->enc ? NULL : opts->enc->supported_framerates;
                                   ...
                                   OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    {
    case AVMEDIA_TYPE_VIDEO:
        return configure_output_video_filter(fg, graph, ofilter, out);
    case AVMEDIA_TYPE_AUDIO:
        return configure_output_audio_filter(fg, graph, ofilter, out);
    default:
        av_assert0(0);
        return 0;
    }
...
static int ifilter_parameters_from_frame(InputFilter *ifilter, const AVFrame *frame)
{
    ...
    ifp->time_base = (ifp->type == AVMEDIA_TYPE_AUDIO) ? (AVRational){1, frame->sample_rate} : (ifp->opts.flags & IFILTER_FLAG_CFR) ? av_inv_q(ifp->opts.framerate)
                                                                                                                                    : frame->time_base;
...
finish:
    ...
    if (*nb_frames > (*nb_frames_prev && fps->last_dropped) + (*nb_frames > *nb_frames_prev))
    {
        ...
        {
            av_log(ofp, AV_LOG_ERROR, "%lld frame duplication too large, skipping\n", *nb_frames - 1);
            atomic_fetch_add(&ofilter->nb_frames_drop, 1);
            *nb_frames = 0;
            return;
        }
                           ...
                           AVFrame *frame)
{
    ...
    for (int64_t i = 0; i < nb_frames; i++)
    {
        ...
        {
            frame->pts = (frame->pts == AV_NOPTS_VALUE) ? ofp->next_pts : av_rescale_q(frame->pts, frame->time_base, ofp->tb_out) - av_rescale_q(ofp->ts_offset, AV_TIME_BASE_Q, ofp->tb_out);
