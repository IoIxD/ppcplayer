    ...
    if (ost->enc_ctx)
    {
        ...
        *vsync_method = VSYNC_AUTO;
        ...
        if (*vsync_method == VSYNC_AUTO)
        {
            ...
            {
                *vsync_method = (oc->oformat->flags & AVFMT_VARIABLE_FPS) ? ((oc->oformat->flags & AVFMT_NOTIMESTAMPS) ? VSYNC_PASSTHROUGH : VSYNC_VFR) : VSYNC_CFR;
            }
                   ...
                   OutputStream **post)
{
        ...
        (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO))
    {
        ...
        OutputFilterOptions opts = {
            .enc = enc,
            .name = name,
            .format = (type == AVMEDIA_TYPE_VIDEO) ? ost->enc_ctx->pix_fmt : ost->enc_ctx->sample_fmt,
            ...
            .ts_offset = mux->of.start_time == AV_NOPTS_VALUE ? 0 : mux->of.start_time,
...
static int map_auto_audio(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int j = 0; j < nb_input_files; j++)
    {
        ...
        for (int i = 0; i < ifile->nb_streams; i++)
        {
            ...
            score = ist->st->codecpar->ch_layout.nb_channels + 100000000 * !!(ist->st->event_flags & AVSTREAM_EVENT_FLAG_NEW_PACKETS) + 5000000 * !!(ist->st->disposition & AV_DISPOSITION_DEFAULT);
            if (score > file_best_score)
            {
                file_best_score = score;
                file_best_ist = ist;
...
static int of_add_attachments(Muxer *mux, const OptionsContext *o)
{
    ...
    for (int i = 0; i < o->nb_attachments; i++)
    {
        ...
        {
            av_log(mux, AV_LOG_FATAL, "Could not read all %lld bytes for "
                                      "attachment file %s\n",
                   len, o->attachments[i]);
            err = AVERROR(EIO);
        }
...
static int of_parse_group_token(Muxer *mux, const char *token, char *ptr)
{
    ...
    const AVDictionaryEntry *e;
    const AVOption opts[] = {
        {"type", "Set group type", offsetof(AVStreamGroup, type), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, .unit = "type"},
        {"iamf_audio_element", NULL, 0, AV_OPT_TYPE_CONST, {.i64 = AV_STREAM_GROUP_PARAMS_IAMF_AUDIO_ELEMENT}, .unit = "type"},
        {"iamf_mix_presentation", NULL, 0, AV_OPT_TYPE_CONST, {.i64 = AV_STREAM_GROUP_PARAMS_IAMF_MIX_PRESENTATION}, .unit = "type"},
        {NULL},
    };
    const AVClass class = {
    ...
    while (e = av_dict_get(dict, "st", e, 0))
    {
        ...
        {
            av_log(mux, AV_LOG_ERROR, "Invalid stream index %lld\n", idx);
            ret = AVERROR(EINVAL);
            goto end;
        }
    ...
    while (e = av_dict_get(dict, "stg", e, 0))
    {
        ...
        {
            av_log(mux, AV_LOG_ERROR, "Invalid stream group index %lld\n", idx);
            ret = AVERROR(EINVAL);
            goto end;
        }
