...
static BOOL WINAPI CtrlHandler(DWORD fdwCtrlType)
{
    ...
    default:
        av_log(NULL, AV_LOG_ERROR, "Received unknown windows signal %lld\n", fdwCtrlType);
        return FALSE;
    }
}
#endif
...
static void ffmpeg_cleanup(int ret)
{
    if (do_benchmark)
    {
        ...
        av_log(NULL, AV_LOG_INFO, "bench: maxrss=%lldKiB\n", maxrss);
    }
        ...
        *dst = av_buffer_create((uint8_t *)fd, sizeof(*fd),
        ...
        if (src)
        {
            ...
            if (fd_src->par_enc)
            {
                ...
                ret = fd->par_enc ? avcodec_parameters_copy(fd->par_enc, fd_src->par_enc) : AVERROR(ENOMEM);
                if (ret < 0)
                {
                    av_buffer_unref(dst);
                    av_buffer_unref(&src);
                         ...
                         void *logctx, int decode)
{
    ...
    const int flag = decode ? AV_OPT_FLAG_DECODING_PARAM : AV_OPT_FLAG_ENCODING_PARAM;
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost))
    {
        ...
        if (!vid && ost->type == AVMEDIA_TYPE_VIDEO && ost->filter)
        {
            ...
            av_bprintf(&buf, "frame=%5lld fps=%3.*f q=%3.1f ",
                       frame_number, fps < 9.95, fps, q);
            av_bprintf(&buf_script, "frame=%lld\n", frame_number);
    ...
    {
        av_bprintf(&buf, "%s%02lld:%02d:%02d.%02d ",
    ...
    if (total_size < 0)
        ...
        av_bprintf(&buf_script, "total_size=%lld\n", total_size);
    if (pts == AV_NOPTS_VALUE)
    {
    ...
    {
        av_bprintf(&buf_script, "out_time_us=%lld\n", pts);
        av_bprintf(&buf_script, "out_time_ms=%lld\n", pts);
        av_bprintf(&buf_script, "out_time=%s%02lld:%02d:%02d.%06d\n",
                   hours_sign, hours, mins, secs, us);
    }

    if (nb_frames_dup || nb_frames_drop)
        av_bprintf(&buf, " dup=%lld drop=%lld", nb_frames_dup, nb_frames_drop);
    av_bprintf(&buf_script, "dup_frames=%lld\n", nb_frames_dup);
    av_bprintf(&buf_script, "drop_frames=%lld\n", nb_frames_drop);
...
int main(int argc, char **argv)
{
    ...
    ret = received_nb_signals ? 255 : (ret == FFMPEG_ERROR_RATE_EXCEEDED) ? 69
                                                                          : ret;
