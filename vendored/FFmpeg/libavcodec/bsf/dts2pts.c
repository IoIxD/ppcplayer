...
static int h264_queue_frame(AVBSFContext *ctx, AVPacket *pkt, int poc, int *queued)
{
    ...
    if (ret < 0)
        return ret;
    av_log(ctx, AV_LOG_DEBUG, "Queueing frame with POC %d, GOP %d, dts %lld\n",
           poc, s->gop, pkt->dts);
    s->nb_frame += poc_diff;
...
static int h264_filter(AVBSFContext *ctx)
{
    ...
    for (int i = 0; i < au->nb_units; i++)
    {
        ...
        case H264_NAL_SLICE:
        {
            ...
            h264->picture_structure = sps->frame_mbs_only_flag ? 3 : (header->field_pic_flag ? header->field_pic_flag + header->bottom_field_flag : 3);
...
static int dts2pts_filter(AVBSFContext *ctx, AVPacket *out)
{
    ...
    else if (s->eof && frame.poc > INT_MIN)
    {
        ...
        if (poc_node && poc_node->poc == dup.poc)
        {
            ...
            if (!ret)
                av_log(ctx, AV_LOG_DEBUG, "Queueing frame for POC %d, GOP %d, dts %lld, "
                                          "generated from POC %d, GOP %d, dts %lld, duration %lld\n",
        ...
        av_log(ctx, AV_LOG_WARNING, "No timestamp for POC %d in tree\n", frame.poc);
    av_log(ctx, AV_LOG_DEBUG, "Returning frame for POC %d, GOP %d, dts %lld, pts %lld\n",
