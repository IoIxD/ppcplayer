                                       ...
                                       VP9RawReorderFrame *last_frame)
{
    ...
    else if (frame->needs_output)
    {
        ...
        {
            av_log(bsf, AV_LOG_DEBUG, "Output frame %lld "
                                      "(%lld) for later display.\n",
                   frame->sequence, frame->pts);
        }
        else
        {
            av_log(bsf, AV_LOG_DEBUG, "Output unshown frame "
                                      "%lld (%lld) to keep order.\n",
                   frame->sequence, frame->pts);
        }
    ...
    else
    {
        ...
        av_log(bsf, AV_LOG_DEBUG, "Display frame %lld "
                                  "(%lld) from slot %d.\n",
...
static int vp9_raw_reorder_filter(AVBSFContext *bsf, AVPacket *out)
{
    ...
    else
    {
        ...
        if (frame->show_existing_frame)
            av_log(bsf, AV_LOG_DEBUG, "Show frame %lld "
                                      "(%lld): show %u.\n",
                   frame->sequence,
                   frame->pts, frame->frame_to_show);
        else
            av_log(bsf, AV_LOG_DEBUG, "New frame %lld "
                                      "(%lld): type %u show %u refresh %02x.\n",
