...
static int dv_error_marker_filter(AVBSFContext *ctx, AVPacket *pkt)
{
    ...
    av_log(ctx, AV_LOG_DEBUG, "%8lld Replaced %5d blocks by color %X\n", pkt->pts, match_count, AV_RB32(s->color_rgba));
...
static const AVOption options[] = {
    ...
    {"sta", "specify which error status value to match", OFFSET(sta), AV_OPT_TYPE_FLAGS, {.i64 = 0xFFFE}, 0, 0xFFFF, FLAGS, .unit = "sta"},
