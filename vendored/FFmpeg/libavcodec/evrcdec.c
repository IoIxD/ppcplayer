...
static void warn_insufficient_frame_quality(AVCodecContext *avctx,
                                            const char *message)
{
    av_log(avctx, AV_LOG_WARNING, "Frame #%lld, %s\n",
...
static void interpolate_delay(float *dst, float current, float prev, int index)
{
    ...
    dst[0] = (1.0 - d_interpolation_factors[index]) * prev + d_interpolation_factors[index] * current;
    dst[1] = (1.0 - d_interpolation_factors[index + 1]) * prev + d_interpolation_factors[index + 1] * current;
    dst[2] = (1.0 - d_interpolation_factors[index + 2]) * prev + d_interpolation_factors[index + 2] * current;
}
                             ...
                             int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    if (e->bitrate <= SILENCE || e->bitrate == RATE_QUARTER)
        goto erasure;
    if (e->bitrate == RATE_QUANT && e->last_valid_bitrate == RATE_FULL && !e->prev_error_flag)
        goto erasure;
    ...
    for (i = 0; i < NB_SUBFRAMES; i++)
    {
        ...
        if (e->bitrate != RATE_QUANT)
        {
            ...
            f = exp((e->bitrate == RATE_HALF ? 0.5 : 0.25) * (e->frame.fcb_gain[i] + 1));
