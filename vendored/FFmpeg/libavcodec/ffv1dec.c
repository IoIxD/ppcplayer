...
static int decode_slice_header(const FFV1Context *f, FFV1Context *fs)
{
    ...
    av_assert0((unsigned)fs->slice_x + (uint64_t)fs->slice_width <= f->width && (unsigned)fs->slice_y + (uint64_t)fs->slice_height <= f->height);
    ...
    if (f->colorspace == 0)
    {
        ...
        else if (f->avctx->bits_per_raw_sample == 10 && !f->transparency)
        {
            ...
            case 0x00:
                ...
                break;
            case 0x01:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV440P10;
                break;
            case 0x10:
        ...
        else if (f->avctx->bits_per_raw_sample == 12 && !f->transparency)
        {
            ...
            {
            case 0x00:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV444P12;
                break;
            case 0x01:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV440P12;
                break;
            case 0x10:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV422P12;
                break;
            case 0x11:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV420P12;
                break;
            }
        }
        else if (f->avctx->bits_per_raw_sample == 12 && f->transparency)
        {
            ...
            {
            case 0x00:
                f->avctx->pix_fmt = AV_PIX_FMT_YUVA444P12;
                break;
            case 0x10:
                f->avctx->pix_fmt = AV_PIX_FMT_YUVA422P12;
                break;
            }
        }
        else if (f->avctx->bits_per_raw_sample == 14 && !f->transparency)
        {
            ...
            {
            case 0x00:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV444P14;
                break;
            case 0x10:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV422P14;
                break;
            case 0x11:
                f->avctx->pix_fmt = AV_PIX_FMT_YUV420P14;
                break;
            }
        }
    ...
    for (i = f->slice_count - 1; i >= 0; i--)
    {
        ...
        if (f->ec)
        {
            ...
            if (crc)
            {
                ...
                {
                    av_log(f->avctx, AV_LOG_ERROR, "at %lld\n", ts);
                }
                else
                {
                    av_log(f->avctx, AV_LOG_ERROR, "\n");
    ...
    for (i = f->slice_count - 1; i >= 0; i--)
    {
        ...
        if (fs->slice_damaged && f->last_picture.f)
        {
            ...
            for (j = 0; j < desc->nb_components; j++)
            {
                ...
                dst[j] = p->data[j] + p->linesize[j] * (fs->slice_y >> sv) + ((fs->slice_x >> sh) << pixshift);
