...
static double hom(double f)
{
    ...
    return -3.64 * pow(f1, -0.8) + 6.8 * exp(-0.6 * (f1 - 3.4) * (f1 - 3.4)) - 6.0 * exp(-0.15 * (f1 - 8.7) * (f1 - 8.7)) - 0.0006 * (f1 * f1) * (f1 * f1);
}
...
static int encode_init(AVCodecContext *avctx)
{
    ...
    {
        av_log(avctx, AV_LOG_ERROR, "Bit rate %lld not supported.", avctx->bit_rate);
        return AVERROR(EINVAL);
    }
