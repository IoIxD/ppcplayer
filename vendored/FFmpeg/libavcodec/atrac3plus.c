...
#define CODING_VLC_DIFF                                \
    (ref_chan->qu_tab_idx[i] +                         \
     get_vlc2(gb, vlc_tab->table, vlc_tab->bits, 1)) & \
        mask
                               ...
                               AVCodecContext *avctx)
{
    ...
    for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++)
        if (band_has_tones[sb])
        {
            ...
            {
                av_log(avctx, AV_LOG_ERROR,
                       "Too many tones: %d (max. 48), frame: %lld!\n",
                                   ...
                                   int ch_num, int band_has_tones[])
{
    ...
    { /* mode 1: VLC modulo delta to master (slave only) */
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++)
        {
            if (!band_has_tones[sb] || !dst[sb].num_wavs)
                continue;
            ...
            for (i = 0; i < dst[sb].num_wavs; i++)
            {
                ...
                pred = (i < ref[sb].num_wavs) ? iwav[i].freq_index : (ref[sb].num_wavs ? iwav[ref[sb].num_wavs - 1].freq_index : 0);
    ...
    case 2: /** VLC modulo delta to master (slave only) */
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++)
        {
            ...
            for (i = 0; i < dst[sb].num_wavs; i++)
            {
                ...
                pred = refwaves[dst[sb].start_index + i] >= 0 ? ctx->waves_info->waves[refwaves[dst[sb].start_index + i]].amp_sf : 34;
