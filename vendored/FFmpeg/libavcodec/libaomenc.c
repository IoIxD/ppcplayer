                         ...
                         struct aom_codec_enc_cfg *enccfg)
{
    ...
    ctx->tile_cols_log2 = ctx->tile_cols == 1 ? 0 : av_log2(ctx->tile_cols - 1) + 1;
    ctx->tile_rows_log2 = ctx->tile_rows == 1 ? 0 : av_log2(ctx->tile_rows - 1) + 1;
...
static av_cold void av1_init_static(FFCodec *codec)
{
    ...
    if (codec_caps & AOM_CODEC_CAP_HIGHBITDEPTH)
        codec->p.pix_fmts = supports_monochrome ? av1_pix_fmts_highbd_with_gray : av1_pix_fmts_highbd;
    else
        codec->p.pix_fmts = supports_monochrome ? av1_pix_fmts_with_gray : av1_pix_fmts;
}
...
static const AVOption options[] = {
    ...
    FF_AV1_PROFILE_OPTS{"still-picture", "Encode in single frame mode (typically used for still AVIF images).", OFFSET(still_picture), AV_OPT_TYPE_BOOL, {.i64 = 0}, -1, 1, VE},
