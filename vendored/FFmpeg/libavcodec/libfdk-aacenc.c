...
/* The version macro is introduced the same time as the 7.1 support, so this
   should suffice. */
    ...
    else
    {
        ...
        {
            av_log(avctx, AV_LOG_ERROR, "Unable to set the bitrate %lld: %s\n",
    ...
    if ((err = aacEncoder_SetParam(s->handle, AACENC_TRANSMUX,
                                   avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER ? TT_MP4_RAW : s->latm ? TT_MP4_LOAS
                                                                                                     : TT_MP4_ADTS)) != AACENC_OK)
    {
