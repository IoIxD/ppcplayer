...
#define STATS_OUT_SIZE 21 * MAX_N * 3 + 4
        avctx->stats_out = av_mallocz(STATS_OUT_SIZE); // 21*256*3(%ulld ) + 3(\n) + 1(0) = 16132
        if (!avctx->stats_out)
            return AVERROR(ENOMEM);
    }
    s->version = 2;
                        ...
                        const AVFrame *p, int *got_packet)
{
    ...
    if ((s->flags & AV_CODEC_FLAG_PASS1) && (s->picture_number & 31) == 0)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            for (j = 0; j < s->vlc_n; j++)
            {
                snprintf(p, end - p, "%ulld ", s->stats[i][j]);
                p += strlen(p);
                s->stats[i][j] = 0;
            }
            snprintf(p, end - p, "\n");
...
static const AVOption options[] = {
    ...
    /* Common options */
    {"non_deterministic", "Allow multithreading for e.g. context=1 at the expense of determinism", OFFSET(non_determ), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, VE},
