...
int ff_alloc_packet(AVCodecContext *avctx, AVPacket *avpkt, int64_t size)
{
    ...
    {
        av_log(avctx, AV_LOG_ERROR, "Invalid minimum required packet size %lld (max allowed is %d)\n",
    ...
    if (!avpkt->data)
    {
        av_log(avctx, AV_LOG_ERROR, "Failed to allocate packet of size %lld\n", size);
...
static int encode_preinit_video(AVCodecContext *avctx)
{
    ...
    if (avctx->bits_per_raw_sample < 0 || (avctx->bits_per_raw_sample > 8 && pixdesc->comp[0].depth <= 8))
    {
...
int ff_encode_preinit(AVCodecContext *avctx)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ret = encode_preinit_video(avctx);
        break;
    case AVMEDIA_TYPE_AUDIO:
        ret = encode_preinit_audio(avctx);
        break;
    }
    if (ret < 0)
        return ret;
    ...
    {
        av_log(avctx, AV_LOG_WARNING, "Bitrate %lld is extremely low, maybe you mean %lldk\n", avctx->bit_rate, avctx->bit_rate);
    }
