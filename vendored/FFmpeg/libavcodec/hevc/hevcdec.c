                             ...
                             const HEVCSPS *sps, GetBitContext *gb)
{
    ...
    if (sps->chroma_format_idc != 0)
    {
        ...
        {
            av_log(logctx, AV_LOG_ERROR, "chroma_log2_weight_denom %lld is invalid\n", chroma_log2_weight_denom);
            return AVERROR_INVALIDDATA;
        }
    ...
    for (i = 0; i < sh->nb_refs[L0]; i++)
    {
        ...
        if (chroma_weight_l0_flag[i])
        {
            for (j = 0; j < 2; j++)
            {
                ...
                sh->chroma_offset_l0[i][j] = av_clip((delta_chroma_offset_l0 - ((128 * sh->chroma_weight_l0[i][j]) >> sh->chroma_log2_weight_denom) + 128), -128, 127);
            }
        }
        else
        {
        ...
        for (i = 0; i < sh->nb_refs[L1]; i++)
        {
            ...
            if (chroma_weight_l1_flag[i])
            {
                for (j = 0; j < 2; j++)
                {
                    ...
                    sh->chroma_offset_l1[i][j] = av_clip((delta_chroma_offset_l1 - ((128 * sh->chroma_weight_l1[i][j]) >> sh->chroma_log2_weight_denom) + 128), -128, 127);
                }
            }
            else
            {
                          ...
                          int rx, int ry)
{
    ...
    {
        int log2_sao_offset_scale = c_idx == 0 ? pps->log2_sao_offset_scale_luma : pps->log2_sao_offset_scale_chroma;
    ...
    else
    {
        ...
        if (!pcm_flag)
        {
            ...
            if (rqt_root_cbf)
            {
                ...
                lc->cu.max_trafo_depth = lc->cu.pred_mode == MODE_INTRA ? sps->max_transform_hierarchy_depth_intra + lc->cu.intra_split_flag : sps->max_transform_hierarchy_depth_inter;
...
static int decode_nal_units(HEVCContext *s, const uint8_t *buf, int length)
{
    ...
    if (s->pkt.nb_nals > 1 && s->pkt.nals[s->pkt.nb_nals - 1].type == HEVC_NAL_UNSPEC62 &&
        s->pkt.nals[s->pkt.nb_nals - 1].size > 2 && !s->pkt.nals[s->pkt.nb_nals - 1].nuh_layer_id && !s->pkt.nals[s->pkt.nb_nals - 1].temporal_id)
    {
    ...
    for (i = 0; i < s->pkt.nb_nals; i++)
    {
        ...
        if (s->avctx->skip_frame >= AVDISCARD_ALL ||
            (s->avctx->skip_frame >= AVDISCARD_NONREF && ff_hevc_nal_is_nonref(nal->type)) || nal->nuh_layer_id > 0)
            continue;
