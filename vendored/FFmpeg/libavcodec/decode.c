                                 ...
                                 int64_t reordered_pts, int64_t dts)
{
    ...
    if ((dc->pts_correction_num_faulty_pts <= dc->pts_correction_num_faulty_dts || dts == AV_NOPTS_VALUE) && reordered_pts != AV_NOPTS_VALUE)
        pts = reordered_pts;
    else
        pts = dts;
    ...
    if (avctx->flags & AV_CODEC_FLAG_DROPCHANGED)
    {
        ...
        if (avctx->frame_num > 1)
        {
            ...
            if (changed)
            {
                ...
                av_log(avctx, AV_LOG_INFO, "dropped changed frame #%lld pts %lld drop count: %d \n",
...
int ff_get_buffer(AVCodecContext *avctx, AVFrame *frame, int flags)
{
    ...
    else if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
        ...
        {
            av_log(avctx, AV_LOG_ERROR, "samples per frame %d, exceeds max_samples %lld\n", frame->nb_samples, avctx->max_samples);
            ret = AVERROR(EINVAL);
            goto fail;
        }
    }
