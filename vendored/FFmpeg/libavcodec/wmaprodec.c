...
static av_cold int decode_init(WMAProDecodeCtx *s, AVCodecContext *avctx, int num_stream)
{
    ...
    for (i = 0; i < num_possible_block_sizes; i++)
    {
        ...
        {
            int x;
            int offset = ((s->sfb_offsets[i][b] + s->sfb_offsets[i][b + 1] - 1) << i) >> 1;
    ...
    for (i = 0; i < WMAPRO_BLOCK_SIZES; i++)
    {
        const float scale = 1.0 / (1 << (WMAPRO_BLOCK_MIN_BITS + i - 1)) / (1ll << (s->bits_per_sample - 1));
    ...
    for (i = 0; i < num_possible_block_sizes; i++)
    {
        ...
        int cutoff = (440 * block_size + 3LL * (s->avctx->sample_rate >> 1) - 1) / s->avctx->sample_rate;
                                        ...
                                        WMAProChannelGrp *chgroup)
{
    ...
    memset(chgroup->decorrelation_matrix, 0, s->nb_channels * s->nb_channels * sizeof(*chgroup->decorrelation_matrix));
...
static int decode_channel_transform(WMAProDecodeCtx *s)
{
    ...
    if (s->nb_channels > 1)
    {
        ...
        for (s->num_chgroups = 0; remaining_channels &&
                                  s->num_chgroups < s->channels_for_cur_subframe;
             s->num_chgroups++)
        {
    ...
    */
    ...
    if (transmit_coeffs)
    {
        ...
        for (i = 0; i < s->channels_for_cur_subframe; i++)
        {
            ...
            if (c == s->lfe_channel)
                memset(&s->tmp[cur_subwoofer_cutoff], 0, sizeof(*s->tmp) * (subframe_len - cur_subwoofer_cutoff));
    ...
    *got_frame_ptr = 0;
    ...
    else if (s->packet_done || s->packet_loss)
    {
        ...
        else
        {
            ...
            ff_dlog(avctx, "packet[%lld]: number of frames %d\n", avctx->frame_num, num_frames);
        ...
        if (avctx->codec_id != AV_CODEC_ID_WMAPRO)
        {
            ...
            ff_dlog(avctx, "packet[%lld]: skip packets %d\n", avctx->frame_num, s->skip_packets);
        }

        ff_dlog(avctx, "packet[%lld]: nbpf %x\n", avctx->frame_num,
                num_bits_prev_frame);
