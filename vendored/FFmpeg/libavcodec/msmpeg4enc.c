        ...
        for (level = 0; level <= MAX_LEVEL; level++)
        {
            ...
            for (run = 0; run <= MAX_RUN; run++)
            {
                ...
                for (last = 0; last < 2; last++)
                {
                    ...
                    {
                        size += intra_luma_count * rl_length[i][level][run][last] + intra_chroma_count * rl_length[i + 3][level][run][last] + inter_count * rl_length[i + 3][level][run][last];
                    }
                }
                if (last_size == size + chroma_size)
                    break;
            }
        }
        if (size < best_size)
