                             ...
                             VAAPIEncodePicture *pic)
{
    ...
    av_log(avctx, AV_LOG_DEBUG, "Sync to pic %lld/%lld "
                                "(input surface %#x).\n",
                              ...
                              const FFHWBaseEncodePicture *base_pic)
{
    ...
    av_log(avctx, AV_LOG_DEBUG, "Issuing encode for pic %lld/%lld "
                                "as type %s.\n",
    ...
    else
    {
        ...
        {
            av_log(avctx, AV_LOG_DEBUG, " %lld/%lld",
                               ...
                               const FFHWBaseEncodePicture *base_pic, AVPacket *pkt)
{
    ...
    av_log(avctx, AV_LOG_DEBUG, "Output read for pic %lld/%lld.\n",
           base_pic->display_order, base_pic->encode_order);
...
rc_mode_found:
            ...
            if (avctx->rc_max_rate > 0)
            {
                if (avctx->rc_max_rate < avctx->bit_rate)
                {
                    av_log(avctx, AV_LOG_ERROR, "Invalid bitrate settings: "
                                                "bitrate (%lld) must not be greater than "
                                                "maxrate (%lld).\n",
                           avctx->bit_rate,
                           avctx->rc_max_rate);
                    return AVERROR(EINVAL);
    ...
    if (rc_mode->va_mode != VA_RC_CQP)
    {
        if (rc_mode->mode == RC_MODE_AVBR)
        {
            av_log(avctx, AV_LOG_VERBOSE, "RC target: %lld bps "
    ...
    if (rc_mode->hrd)
    {
        av_log(avctx, AV_LOG_VERBOSE, "RC buffer: %lld bits, "
                                      "initial fullness %lld bits.\n",
                                                          ...
                                                          uint32_t slice_structure)
{
        ...
        ctx->tile_cols > ctx->slice_block_cols)
    {
        ...
        ctx->tile_rows = ctx->tile_rows > ctx->slice_block_rows ? ctx->slice_block_rows : ctx->tile_rows;
        ctx->tile_cols = ctx->tile_cols > ctx->slice_block_cols ? ctx->slice_block_cols : ctx->tile_cols;
