...
void ff_write_pass1_stats(MpegEncContext *s)
{
    snprintf(s->avctx->stats_out, 256,
             ...
             "fcode:%d bcode:%d mc-var:%lld var:%lld icount:%d hbits:%d;\n",
        ...
        if (min_rate)
        {
            ...
            q_limit = bits2qp(rce,
                              FFMAX((min_rate - buffer_size + rcc->buffer_index) *
                                        s->avctx->rc_min_vbv_overflow_use,
                                    1));
...
float ff_rate_estimate_qscale(MpegEncContext *s, int dry_run)
{
    ...
    if (s->avctx->flags & AV_CODEC_FLAG_PASS2)
    {
        ...
        ff_dlog(s, "%f %f %f last:%d var:%lld type:%d//\n", q, rce->new_qscale,
    ...
    {
        av_log(s->avctx, AV_LOG_DEBUG,
               "%c qp:%d<%2.1f<%d %d want:%lld total:%lld comp:%f st_q:%2.2f "
               "size:%d var:%lld/%lld br:%lld fps:%d\n",
