...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **options)
{
    ...
    if ((avctx->coded_width || avctx->coded_height || avctx->width || avctx->height) && (av_image_check_size2(avctx->coded_width, avctx->coded_height, avctx->max_pixels, AV_PIX_FMT_NONE, 0, avctx) < 0 || av_image_check_size2(avctx->width, avctx->height, avctx->max_pixels, AV_PIX_FMT_NONE, 0, avctx) < 0))
    {
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO && !avctx->ch_layout.nb_channels && !(codec->capabilities & AV_CODEC_CAP_CHANNEL_CONF))
    {
...
void avcodec_string(char *buf, int buf_size, AVCodecContext *enc, int encode)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
    {
        ...
        av_bprintf(&bprint, "%s%s", separator,
                   enc->pix_fmt == AV_PIX_FMT_NONE ? "none" : unknown_if_null(av_get_pix_fmt_name(enc->pix_fmt)));
    ...
    if (bitrate != 0)
    {
        av_bprintf(&bprint, ", %lld kb/s", bitrate / 1000);
    }
    else if (enc->rc_max_rate > 0)
    {
        av_bprintf(&bprint, ", max. %lld kb/s", enc->rc_max_rate / 1000);
    }
}
