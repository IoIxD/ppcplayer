                        ...
                        const char *name, const char *sep, TiffContext *s, AVFrame *frame)
{
    ...
    {
    case TIFF_DOUBLE:
        return ff_tadd_doubles_metadata(count, name, sep, &s->gb, s->le, &frame->metadata);
    case TIFF_SHORT:
        return ff_tadd_shorts_metadata(count, name, sep, &s->gb, s->le, 0, &frame->metadata);
    case TIFF_STRING:
        return ff_tadd_string_metadata(count, name, &s->gb, s->le, &frame->metadata);
    default:
        return AVERROR_INVALIDDATA;
    };
}
                            ...
                            int strip_start, int is_yuv)
{
    ...
    {
        av_log(s->avctx, AV_LOG_ERROR,
               "Uncompressing failed (%ulld of %ulld) with error %d\n", outlen,
                             ...
                             const uint8_t *src, int size, int strip_start, int lines)
{
    ...
    if (is_yuv)
    {
        int bytes_per_row = (((s->width - 1) / s->subsampling[0] + 1) * s->bpp *
                                 s->subsampling[0] * s->subsampling[1] +
                             7) >>
                            3;
...
#define ADD_METADATA(count, name, sep)                                         \
    ...
    case TIFF_GEO_KEY_DIRECTORY:
        ...
        if (bytestream2_get_bytes_left(&s->gb) < s->geotag_count * sizeof(int16_t) * 4 || s->geotag_count == 0)
        {
            s->geotag_count = 0;
            return -1;
        }
    ...
    case TIFF_GEO_DOUBLE_PARAMS:
        ...
        for (i = 0; i < s->geotag_count; i++)
        {
            ...
            {
                if (s->geotags[i].count == 0 || s->geotags[i].offset + s->geotags[i].count > count)
                {
