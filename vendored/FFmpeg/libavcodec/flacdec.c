                               ...
                               int bps)
{
    ...
    if ((s->buggy_lpc && s->stream_info.bps <= 16) || (!s->buggy_lpc && bps <= 16 && bps + coeff_prec + av_log2(pred_order) <= 32))
    {
...
static int decode_frame(FLACContext *s)
{
    ...
    if (s->stream_info.channels && fi.channels != s->stream_info.channels && s->got_streaminfo)
    {
    ...
    *got_frame_ptr = 0;
    ...
    if ((s->avctx->err_recognition & (AV_EF_CRCCHECK | AV_EF_COMPLIANT)) &&
        av_crc(av_crc_get_table(AV_CRC_16_ANSI),
               0, buf, bytes_read))
    {
        av_log(s->avctx, AV_LOG_ERROR, "CRC error at PTS %lld\n", avpkt->pts);
