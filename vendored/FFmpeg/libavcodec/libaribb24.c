...
static int libaribb24_handle_regions(AVCodecContext *avctx, AVSubtitle *sub)
{
    ...
    while (region)
    {
        ...
        {
            av_bprintf(&buf, "{\\fscx%lld\\fscy%lld}",
                             ...
                             int *got_sub_ptr, const AVPacket *pkt)
{
    ...
    sub->end_display_time = subtitle_duration ? av_rescale_q(subtitle_duration,
                                                             AV_TIME_BASE_Q,
                                                             (AVRational){1, 1000})
                                              : UINT32_MAX;

    av_log(avctx, AV_LOG_DEBUG,
           "Result: '%s' (size: %zu, pkt_pts: %lld, sub_pts: %lld "
...
static const AVOption options[] = {
    {"aribb24-base-path", "set the base path for the libaribb24 library", OFFSET(aribb24_base_path), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, SD},
    {"aribb24-skip-ruby-text", "skip ruby text blocks during decoding", OFFSET(aribb24_skip_ruby), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, SD},
    {"default_profile", "default profile to use if not specified in the stream parameters", OFFSET(default_profile), AV_OPT_TYPE_INT, {.i64 = AV_PROFILE_UNKNOWN}, AV_PROFILE_UNKNOWN, AV_PROFILE_ARIB_PROFILE_C, SD, .unit = "profile"},
