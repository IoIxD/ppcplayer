                       ...
                       const AVMasteringDisplayMetadata *mdcv)
{
    ...
    snprintf(buf, sizeof(buf),
             "G(%lld,%lld)B(%lld,%lld)R(%lld,%lld)"
             "WP(%lld,%lld)L(%lld,%lld)",
...
static av_cold int libx265_encode_init(AVCodecContext *avctx)
{
    ...
    // 4:2:0, 4:2:2
    case 1:
        ctx->params->internalCsp = desc->log2_chroma_h == 1 ? X265_CSP_I420 : X265_CSP_I422;
        break;
    default:
        av_log(avctx, AV_LOG_ERROR,
                                ...
                                const AVFrame *pic, int *got_packet)
{
    ...
    if (pic)
    {
        ...
        x265pic.sliceType = pic->pict_type == AV_PICTURE_TYPE_I ? (ctx->forced_idr ? X265_TYPE_IDR : X265_TYPE_I) : pic->pict_type == AV_PICTURE_TYPE_P ? X265_TYPE_P
