...
typedef struct OMXContext
{
    void *lib;
    void *lib2;
    OMX_ERRORTYPE (*ptr_Init)
    (void);
    OMX_ERRORTYPE (*ptr_Deinit)
    (void);
    OMX_ERRORTYPE (*ptr_ComponentNameEnum)
    (OMX_STRING, OMX_U32, OMX_U32);
    OMX_ERRORTYPE (*ptr_GetHandle)
    (OMX_HANDLETYPE *, OMX_STRING, OMX_PTR, OMX_CALLBACKTYPE *);
    OMX_ERRORTYPE (*ptr_FreeHandle)
    (OMX_HANDLETYPE);
    OMX_ERRORTYPE (*ptr_GetComponentsOfRole)
    (OMX_STRING, OMX_U32 *, OMX_U8 **);
    OMX_ERRORTYPE (*ptr_GetRolesOfComponent)
    (OMX_STRING, OMX_U32 *, OMX_U8 **);
    void (*host_init)(void);
} OMXContext;
...
static av_cold void cleanup(OMXCodecContext *s)
{
    ...
    ff_ // pthread_free(s, omx_codec_context_offsets);
}
...
static av_cold int omx_encode_init(AVCodecContext *avctx)
{
    ...
    ret = ff_ // pthread_init(s, omx_codec_context_offsets);
        if (ret < 0) return ret;
