...
typedef struct Vp3DecodeContext
{
    ...
    DECLARE_ALIGNED(16, int16_t, block)
    [64];
    int flipped_image;
    int last_slice_end;
...
#define TOKEN_COEFF(coeff) (((coeff) * 4) + 2)
     ...
     * index into them */
    DECLARE_ALIGNED(16, int16_t, qmat)
    ...
    DECLARE_ALIGNED(8, int, bounding_values_array)
    [256 + 2];
...
static void render_slice(Vp3DecodeContext *s, int slice)
{
    ...
    const AVFrame *last_frame = s->last_frame.f ? s->last_frame.f : s->current_frame.f;
    const AVFrame *golden_frame = s->golden_frame.f ? s->golden_frame.f : s->current_frame.f;
                ...
                for (int j = 0; j < 16; j++)
                {
                    ...
                    if (s->all_fragments[i].coding_method != MODE_COPY)
                    {
                            ...
                            (s->all_fragments[i].coding_method != MODE_USING_GOLDEN))
                        {
                            ...
                            if (standard_mc && (src_x < 0 || src_y < 0 ||
                                                src_x + 9 >= plane_width ||
                            ...
                            int *got_frame, AVPacket *avpkt)
{
    ...
    if (s->avctx->debug & FF_DEBUG_PICT_INFO)
        av_log(s->avctx, AV_LOG_INFO, " VP3 %sframe #%lld: Q index = %d\n",
