                        ...
                        uint8_t *encbuf, uint8_t *databuf)
{
    ...
    for (col = 0; col < s->cols; col++)
    {
        for (row = 0; row < s->rows; row++)
        {
            ...
            b->height = (row < s->rows - 1) ? s->block_height : s->image_height - row * s->block_height;
...
static av_cold int flashsv2_encode_init(AVCodecContext *avctx)
{
    ...
    if (!s->encbuffer || !s->keybuffer || !s->databuffer || !s->current_frame || !s->key_frame)
    {
                                 ...
                                 const AVFrame *p, int *got_packet)
{
    ...
    if (!keyframe && avctx->frame_num > s->last_key_frame + avctx->keyint_min)
    {
        recommend_keyframe(s, &keyframe);
        if (keyframe)
            av_log(avctx, AV_LOG_DEBUG, "Recommending key frame at frame %lld\n", avctx->frame_num);
    }
    ...
    if (keyframe)
    {
        ...
        av_log(avctx, AV_LOG_DEBUG, "Inserting key frame at frame %lld\n", avctx->frame_num);
    }
