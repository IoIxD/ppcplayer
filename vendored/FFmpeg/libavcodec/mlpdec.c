...
typedef struct SubStream
{
    ...
    DECLARE_ALIGNED(32, int32_t, matrix_coeff)
    [MAX_MATRICES][MAX_CHANNELS];
...
typedef struct MLPDecodeContext
{
    ...
    DECLARE_ALIGNED(32, int32_t, sample_buffer)
    [MAX_BLOCKSIZE][MAX_CHANNELS];
...
static int read_major_sync(MLPDecodeContext *m, GetBitContext *gb)
{
     ...
     */
    if (m->avctx->codec_id == AV_CODEC_ID_TRUEHD && m->num_substreams == 4 && m->substream_info >> 7 == 1)
    {
                               ...
                               const uint8_t *buf, unsigned int substr)
{
        ...
        m->max_decoded_substream > substr)
    {
        av_log(m->avctx, AV_LOG_DEBUG,
               "Extracting %d-channel downmix (0x%lld) from substream %d. "
...
static const AVOption options[] = {
    {"downmix", "Request a specific channel layout from the decoder", OFFSET(downmix_layout), AV_OPT_TYPE_CHLAYOUT, {.str = NULL}, .flags = FLAGS},
    {NULL},
};
