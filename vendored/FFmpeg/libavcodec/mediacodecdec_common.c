...
static void mediacodec_buffer_release(void *opaque, uint8_t *data)
{
    ...
    if (!released && (ctx->delay_flush || buffer->serial == atomic_load(&ctx->serial)))
    {
        ...
        av_log(ctx->avctx, AV_LOG_DEBUG,
               "Releasing output buffer %zd (%p) ts=%lld on free() [%d pending]\n",
                                     ...
                                     AVFrame *frame)
{
    ...
    av_log(avctx, AV_LOG_DEBUG,
           "Wrapping output buffer %zd (%p) ts=%lld [%d pending]\n",
                           ...
                           AVPacket *pkt, bool wait)
{
    ...
    while (offset < pkt->size || (need_draining && !s->draining))
    {
        ...
        av_log(avctx, AV_LOG_TRACE,
               "Queued input buffer %zd size=%zd ts=%lld\n", index, size, pts);
    }
                              ...
                              AVFrame *frame, bool wait)
{
    ...
    if (index >= 0)
    {
        av_log(avctx, AV_LOG_TRACE, "Got output buffer %zd"
                                    " offset=%" PRIi32 " size=%" PRIi32 " ts=%lld flags=%" PRIu32 "\n",
    ...
    else if (ff_AMediaCodec_infoTryAgainLater(codec, index))
    {
        if (s->draining)
        {
            av_log(avctx, AV_LOG_ERROR, "Failed to dequeue output buffer within %lldms "
