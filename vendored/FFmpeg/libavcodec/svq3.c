...
static const struct
{
    ...
    {{0, 0}, {0, 1}, {1, 1}, {2, 1}, {0, 2}, {3, 1}, {4, 1}, {5, 1}, {0, 3}, {1, 2}, {2, 2}, {6, 1}, {7, 1}, {8, 1}, {9, 1}, {0, 4}},
    {{0, 0}, {0, 1}, {1, 1}, {0, 2}, {2, 1}, {0, 3}, {0, 4}, {0, 5}, {3, 1}, {4, 1}, {1, 2}, {1, 3}, {0, 6}, {0, 7}, {0, 8}, {0, 9}}};
                            ...
                            int stride, int qp, int dc)
{
    ...
    if (dc)
    {
        dc = 13 * 13 * (dc == 1 ? 1538U * block[0] : qmul * (block[0] >> 3) / 2);
        block[0] = 0;
    }
                ...
                if (s->mb_x < s->mb_width - 1)
                {
                    ...
                    s->ref_cache[m][scan8[0] + 4 - 1 * 8] =
                        (s->intra4x4_pred_mode[s->mb2br_xy[mb_xy - s->mb_stride + 1] + 6] == -1 ||
                         s->intra4x4_pred_mode[s->mb2br_xy[mb_xy - s->mb_stride]] == -1)
                            ? PART_NOT_AVAILABLE
                            : 1;
                }
                else
...
static int get_buffer(AVCodecContext *avctx, SVQ3Frame *pic)
{
    ...
    ret = ff_get_buffer(avctx, pic->f,
                        (s->pict_type != AV_PICTURE_TYPE_B) ? AV_GET_BUFFER_FLAG_REF : 0);
    if (ret < 0)
        goto fail;
                             ...
                             int *got_frame, AVPacket *avpkt)
{
    ...
    {
        av_log(avctx, AV_LOG_INFO, "frame num %lld incomplete pic x %d y %d left %d\n", avctx->frame_num, s->mb_y, s->mb_x, left);
        // av_hex_dump(stderr, buf+buf_size-8, 8);
    }

    if (left < 0)
    {
        av_log(avctx, AV_LOG_ERROR, "frame num %lld left %d\n", avctx->frame_num, left);
        return -1;
    }
