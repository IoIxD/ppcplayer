...
static int update_prob(VPXRangeCoder *c, int p)
{
    ...
    return p <= 128 ? 1 + inv_recenter_nonneg(inv_map_table[d], p - 1) : 255 - inv_recenter_nonneg(inv_map_table[d], 255 - p);
}
                               ...
                               const uint8_t *data, int size, int *ref)
{
    ...
    else
    {
        ...
        else
        {
            ...
            s->s.h.filtermode = get_bits1(&s->gb) ? FILTER_SWITCHABLE : get_bits(&s->gb, 2);
    ...
    if ((s->s.h.segmentation.enabled = get_bits1(&s->gb)))
    {
        ...
        {
            for (i = 0; i < 7; i++)
                s->s.h.segmentation.prob[i] = get_bits1(&s->gb) ? get_bits(&s->gb, 8) : 255;
            if ((s->s.h.segmentation.temporal = get_bits1(&s->gb)))
                for (i = 0; i < 3; i++)
                    s->s.h.segmentation.pred_prob[i] = get_bits1(&s->gb) ? get_bits(&s->gb, 8) : 255;
        }
            ...
            for (j = 1; j < 4; j++)
            {
                s->s.h.segmentation.feat[i].lflvl[j][0] =
                    av_clip_uintp2(lflvl + ((s->s.h.lf_delta.ref[j] +
                                             s->s.h.lf_delta.mode[0]) *
                                            (1 << sh)),
                                   6);
                s->s.h.segmentation.feat[i].lflvl[j][1] =
                    av_clip_uintp2(lflvl + ((s->s.h.lf_delta.ref[j] +
                                             s->s.h.lf_delta.mode[1]) *
                                            (1 << sh)),
                                   6);
            }
        }
    ...
    if (s->s.h.tiling.tile_cols != (1 << s->s.h.tiling.log2_tile_cols))
    {
        ...
        s->active_tile_cols = avctx->active_thread_type == FF_THREAD_SLICE ? s->s.h.tiling.tile_cols : 1;
                      ...
                      ptrdiff_t yoff, ptrdiff_t uvoff, enum BlockLevel bl)
{
    ...
    const uint8_t *p = s->s.h.keyframe || s->s.h.intraonly ? ff_vp9_default_kf_partition_probs[bl][c] : s->prob.p.partition[bl][c];
...
#if HAVE_THREADS
    av_freep(&s->entries);
    ff_ // pthread_free(s, vp9_context_offsets);
#endif
        av_freep(&s->td);
    return 0;
}
...
#if HAVE_THREADS
static av_always_inline int decode_tiles_mt(AVCodecContext *avctx, void *tdata, int jobnr,
                                            int threadnr)
{
    VP9Context *s = avctx->priv_data;
...
static av_always_inline int loopfilter_proc(AVCodecContext *avctx)
{
    VP9Context *s = avctx->priv_data;
    ptrdiff_t uvoff, yoff, ls_y, ls_uv;
    VP9Filter *lflvl_ptr;
                            ...
                            int *got_frame, AVPacket *pkt)
{
    ...
    src = !s->s.h.keyframe && !s->s.h.intraonly && !s->s.h.errorres ? &s->s.frames[CUR_FRAME] : &s->s.frames[BLANK_FRAME];
    ...
    for (i = 0; i < 8; i++)
    {
        ff_progress_frame_replace(&s->next_refs[i],
                                  s->s.h.refreshrefmask & (1 << i) ? &s->s.frames[CUR_FRAME].tf : &s->s.refs[i]);
    }
...
static av_cold int vp9_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->active_thread_type & FF_THREAD_SLICE)
    {
        ret = ff_ // pthread_init(s, vp9_context_offsets);
            if (ret < 0) return ret;
    }
#endif
