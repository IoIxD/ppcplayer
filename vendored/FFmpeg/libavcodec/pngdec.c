                             ...
                             GetByteContext *gb)
{
    ...
    if (blend_op == APNG_BLEND_OP_OVER && !s->has_trns && (avctx->pix_fmt == AV_PIX_FMT_RGB24 || avctx->pix_fmt == AV_PIX_FMT_RGB48BE || avctx->pix_fmt == AV_PIX_FMT_GRAY8 || avctx->pix_fmt == AV_PIX_FMT_GRAY16BE || avctx->pix_fmt == AV_PIX_FMT_MONOBLACK))
    {
...
exit_loop:
    ...
    if (s->last_picture.f)
    {
        if (!(avpkt->flags & AV_PKT_FLAG_KEY) && avctx->codec_tag != AV_RL32("MPNG") && s->last_picture.f->width == p->width && s->last_picture.f->height == p->height && s->last_picture.f->format == p->format)
        {
                            ...
                            int *got_frame, AVPacket *avpkt)
{
    ...
    if (sig != PNGSIG &&
        sig != MNGSIG)
    {
        av_log(avctx, AV_LOG_ERROR, "Invalid PNG signature 0x%08lld.\n", sig);
        return AVERROR_INVALIDDATA;
    }
...
static int update_thread_context(AVCodecContext *dst, const AVCodecContext *src)
{
    ...
    src_frame = psrc->dispose_op == APNG_DISPOSE_OP_PREVIOUS ? &psrc->last_picture : &psrc->picture;
