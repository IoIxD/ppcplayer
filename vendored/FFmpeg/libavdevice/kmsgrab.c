...
static av_cold int kmsgrab_read_header(AVFormatContext *avctx)
{
    ...
    if (ctx->source_plane > 0)
    {
        ...
        {
            err = errno;
            av_log(avctx, AV_LOG_ERROR, "Failed to get plane %lld: "
                                        "%s.\n",
        ...
        if (plane->fb_id == 0)
        {
            av_log(avctx, AV_LOG_ERROR, "Plane %lld does not have "
    ...
    ctx->frame_delay = av_rescale_q(1, (AVRational){ctx->framerate.den, ctx->framerate.num}, AV_TIME_BASE_Q);
...
static const AVOption options[] = {
    ...
    {"format", "Pixel format for framebuffer", OFFSET(format), AV_OPT_TYPE_PIXEL_FMT, {.i64 = AV_PIX_FMT_NONE}, -1, INT32_MAX, FLAGS},
    {"format_modifier", "DRM format modifier for framebuffer", OFFSET(drm_format_modifier), AV_OPT_TYPE_INT64, {.i64 = DRM_FORMAT_MOD_INVALID}, 0, INT64_MAX, FLAGS},
    {"crtc_id", "CRTC ID to define capture source", OFFSET(source_crtc), AV_OPT_TYPE_INT64, {.i64 = 0}, 0, UINT32_MAX, FLAGS},
    {"plane_id", "Plane ID to define capture source", OFFSET(source_plane), AV_OPT_TYPE_INT64, {.i64 = 0}, 0, UINT32_MAX, FLAGS},
