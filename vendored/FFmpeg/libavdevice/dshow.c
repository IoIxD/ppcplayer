                    ...
                    IPin *pin, int *pformat_set)
{
    ...
    int64_t requested_framerate = ctx->framerate ? ((int64_t)ctx->requested_framerate.den * 10000000) / ctx->requested_framerate.num : 0;
    ...
    for (i = 0; i < n && !format_set; i++)
    {
        ...
        if (devtype == VideoDevice)
        {
            ...
            if (!pformat_set)
            {
                ...
                av_log(avctx, AV_LOG_INFO, "  min s=%lldx%lld fps=%g max s=%lldx%lld fps=%g",
...
dshow_set_audio_buffer_size(AVFormatContext *avctx, IPin *pin)
{
    ...
    props.cbBuffer = (((WAVEFORMATEX *)type->pbFormat)->nAvgBytesPerSec) * ctx->audio_buffer_size / 1000;
