                           ...
                           decklink_direction_t direction)
{
    ...
    {
        if (!ctx->supports_vanc || ctx->dlo->DoesSupportVideoMode(ctx->bmd_mode, ctx->raw_format, bmdVideoOutputVANC, &support, NULL) != S_OK || support != bmdDisplayModeSupported)
        {
...
int ff_decklink_list_formats(AVFormatContext *avctx, decklink_direction_t direction)
{
    ...
    while (itermode->Next(&mode) == S_OK)
    {
        ...
        av_log(avctx, AV_LOG_INFO, "\n\t%.4s\t\t%lldx%lld at %d/%d fps",
               (char *)&format_code, mode->GetWidth(), mode->GetHeight(),
               (int)tb_den, (int)tb_num);
        switch (mode->GetFieldDominance())
        {
        case bmdLowerFieldFirst:
            av_log(avctx, AV_LOG_INFO, " (interlaced, lower field first)");
            break;
        case bmdUpperFieldFirst:
            av_log(avctx, AV_LOG_INFO, " (interlaced, upper field first)");
            break;
        }
        mode->Release();
    }
