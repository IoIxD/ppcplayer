...
setup_crossbar_options(IAMCrossbar *cross_bar, enum dshowDeviceType devtype, AVFormatContext *avctx)
{
    ...
    for (i = 0; i < count_output_pins; i++)
    {
        ...
        av_log(avctx, log_level, "  Crossbar Output pin %d: \"%s\" related output pin: %lld ", i, GetPhysicalPinName(pin_type), related_pin);
        av_log(avctx, log_level, "current input pin: %lld ", route_to_pin);
    ...
    for (i = 0; i < count_input_pins; i++)
    {
        ...
        av_log(avctx, log_level, "related input pin: %lld\n", related_pin);
    }
    return S_OK;
}
