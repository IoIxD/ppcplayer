                           ...
                           int split, int ismf, const char *output_prefix)
{
    ...
    for (i = start_index; i < tracks->nb_tracks; i++)
    {
        ...
        {
            snprintf(filename, sizeof(filename), "%s/Fragments(%s=%lld)",
                     dirname, type, track->offsets[j].time);
            avio_seek(in, track->offsets[j].offset, SEEK_SET);
            if (ismf)
                fprintf(out, "%s %lld", filename, avio_tell(in));
            if (split)
    ...
    if (track->chunks > 0)
    {
        if (track->offsets[track->chunks - 1].duration <= 0)
        {
            fprintf(stderr, "Calculated last chunk duration for track %d "
                            "was non-positive (%lld), probably due to missing "
                            "fragments ",
                    track->track_id,
            ...
            fprintf(stderr, "corrected to %lld\n",
                    track->offsets[track->chunks - 1].duration);
            track->duration = track->offsets[track->chunks - 1].time +
                              track->offsets[track->chunks - 1].duration -
                              track->offsets[0].time;
            fprintf(stderr, "Track duration corrected to %lld\n",
                    track->duration);
        }
    }
    ret = 0;
                               ...
                               const char *type)
{
    ...
    for (i = 0; i < track->chunks; i++)
    {
        ...
        fprintf(out, "\t\t<c n=\"%d\" d=\"%lld\" ",
                i, track->offsets[i].duration);
        if (pos != track->offsets[i].time)
        {
            fprintf(out, "t=\"%lld\" ", track->offsets[i].time);
