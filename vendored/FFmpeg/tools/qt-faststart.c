    ...
    void *context)
{
    ...
    while (end - pos >= ATOM_PREAMBLE_SIZE)
    {
        ...
        {
            fprintf(stderr, "atom size %ulld too small\n", atom.size);
            return -1;
        }
        ...
        {
            fprintf(stderr, "atom size %ulld too big\n", atom.size);
            return -1;
        }
...
static int update_stco_offsets(update_chunk_offsets_context_t *context, atom_t *atom)
{
    ...
    if (atom->size < 8)
    {
        fprintf(stderr, "stco atom size %ulld too small\n", atom->size);
        return -1;
    }
...
static int update_co64_offsets(update_chunk_offsets_context_t *context, atom_t *atom)
{
    ...
    if (atom->size < 8)
    {
        fprintf(stderr, "co64 atom size %ulld too small\n", atom->size);
        return -1;
    }
    ...
    uint64_t *moov_atom_size)
{
    ...
    {
        fprintf(stderr, "could not allocate %ulld bytes for updated moov atom\n",
...
int main(int argc, char *argv[])
{
    ...
    while (!feof(infile))
    {
        ...
        if (atom_type == FTYP_ATOM)
        {
            ...
            {
                fprintf(stderr, "ftyp atom size %ulld too big\n",
                        atom_size);
                goto error_out;
            }
            ...
            if (!ftyp_atom)
            {
                fprintf(stderr, "could not allocate %ulld bytes for ftyp atom\n",
                        atom_size);
                goto error_out;
            }
    ...
    if (!moov_atom)
    {
        fprintf(stderr, "could not allocate %ulld bytes for moov atom\n", atom_size);
        goto error_out;
    }
