...
static const AVOption signature_options[] = {
    {"detectmode", "set the detectmode", OFFSET(mode), AV_OPT_TYPE_INT, {.i64 = MODE_OFF}, 0, NB_LOOKUP_MODE - 1, FLAGS, .unit = "mode"},
    ...
    {"nb_inputs", "number of inputs", OFFSET(nb_inputs), AV_OPT_TYPE_INT, {.i64 = 1}, 1, INT_MAX, FLAGS},
    {"filename", "filename for output files", OFFSET(filename), AV_OPT_TYPE_STRING, {.str = ""}, 0, NB_FORMATS - 1, FLAGS},
    {"format", "set output format", OFFSET(format), AV_OPT_TYPE_INT, {.i64 = FORMAT_BINARY}, 0, 1, FLAGS, .unit = "format"},
    ...
    {"th_d", "threshold to detect one word as similar", OFFSET(thworddist), AV_OPT_TYPE_INT, {.i64 = 9000}, 1, INT_MAX, FLAGS},
    {"th_dc", "threshold to detect all words as similar", OFFSET(thcomposdist), AV_OPT_TYPE_INT, {.i64 = 60000}, 1, INT_MAX, FLAGS},
    {"th_xh", "threshold to detect frames as similar", OFFSET(thl1), AV_OPT_TYPE_INT, {.i64 = 116}, 1, INT_MAX, FLAGS},
    {"th_di", "minimum length of matching sequence in frames", OFFSET(thdi), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, FLAGS},
    {"th_it", "threshold for relation of good to all frames", OFFSET(thit), AV_OPT_TYPE_DOUBLE, {.dbl = 0.5}, 0.0, 1.0, FLAGS},
    {NULL}};
...
static int xml_export(AVFilterContext *ctx, StreamContext *sc, const char *filename)
{
    ...
    fprintf(f, "          <EndMediaTimeOfSpatialRegion>%ulld</EndMediaTimeOfSpatialRegion>\n", sc->coarseend->last->pts);
    fprintf(f, "        </MediaTimeOfSpatialRegion>\n");
    ...
    for (cs = sc->coarsesiglist; cs; cs = cs->next)
    {
        ...
        fprintf(f, "            <StartMediaTimeOfSegment>%ulld</StartMediaTimeOfSegment>\n", cs->first->pts);
        fprintf(f, "            <EndMediaTimeOfSegment>%ulld</EndMediaTimeOfSegment>\n", cs->last->pts);
        fprintf(f, "          </MediaTimeOfSegment>\n");
        for (i = 0; i < 5; i++)
        {
    ...
    for (fs = sc->finesiglist; fs; fs = fs->next)
    {
        ...
        fprintf(f, "          <MediaTimeOfFrame>%ulld</MediaTimeOfFrame>\n", fs->pts);
        /* confidence */
