...
static const AVOption dejudder_options[] = {
    {"cycle", "set the length of the cycle to use for dejuddering", OFFSET(cycle), AV_OPT_TYPE_INT, {.i64 = 4}, 2, 240, .flags = FLAGS},
    {NULL}};
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    else
    {
        ...
        s->new_pts += (s->cycle - 1) * (judbuff[s->i3] - judbuff[s->i1]) + (s->cycle + 1) * (next_pts - judbuff[s->i4]);
    }
    ...
    for (k = 0; k < s->cycle + 2; k++)
        av_log(ctx, AV_LOG_DEBUG, "%lld\t", judbuff[k]);
    av_log(ctx, AV_LOG_DEBUG, "next=%lld, new=%lld\n", next_pts, frame->pts);
