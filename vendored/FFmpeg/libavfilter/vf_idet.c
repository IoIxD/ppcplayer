                           ...
                           int flags)
{
    ...
    snprintf(valuestr, sizeof(valuestr), "%lld.%0*lld",
...
static int filter_frame(AVFilterLink *link, AVFrame *picref)
{
    ...
    if (picref->width != link->w || picref->height != link->h || picref->format != link->format)
    {
...
static av_cold void uninit(AVFilterContext *ctx)
{
    ...
    av_log(ctx, AV_LOG_INFO, "Repeated Fields: Neither:%6lld Top:%6lld Bottom:%6lld\n",
           idet->total_repeats[REPEAT_NONE],
           idet->total_repeats[REPEAT_TOP],
           idet->total_repeats[REPEAT_BOTTOM]);
    av_log(ctx, AV_LOG_INFO, "Single frame detection: TFF:%6lld BFF:%6lld Progressive:%6lld Undetermined:%6lld\n",
           idet->total_prestat[TFF],
           idet->total_prestat[BFF],
           idet->total_prestat[PROGRESSIVE],
           idet->total_prestat[UNDETERMINED]);
    av_log(ctx, AV_LOG_INFO, "Multi frame detection: TFF:%6lld BFF:%6lld Progressive:%6lld Undetermined:%6lld\n",
