...
#define DEFINE_OPTIONS(filt_name, filt_type)                                                                                                                                                      \
    static const AVOption filt_name##_options[] = {                                                                                                                                               \
        {"expr", "set expression determining the output timebase", OFFSET(tb_expr), AV_OPT_TYPE_STRING, {.str = "intb"}, .flags = AV_OPT_FLAG_##filt_type##_PARAM | AV_OPT_FLAG_FILTERING_PARAM}, \
        {"tb", "set expression determining the output timebase", OFFSET(tb_expr), AV_OPT_TYPE_STRING, {.str = "intb"}, .flags = AV_OPT_FLAG_##filt_type##_PARAM | AV_OPT_FLAG_FILTERING_PARAM},   \
        {NULL}}
...
static int64_t rescale_pts(AVFilterLink *inlink, AVFilterLink *outlink, int64_t orig_pts)
{
    ...
    if (av_cmp_q(inlink->time_base, outlink->time_base))
    {
        ...
        av_log(ctx, AV_LOG_DEBUG, "tb:%d/%d pts:%lld -> tb:%d/%d pts:%lld\n",
