...
static int overlay_vaapi_blend(FFFrameSync *fs)
{
    ...
    av_log(avctx, AV_LOG_DEBUG, "Filter main: %s, %ux%u (%lld).\n",
    ...
    if (input_overlay)
    {
        av_log(avctx, AV_LOG_DEBUG, "Filter overlay: %s, %ux%u (%lld).\n",
...
static const AVOption overlay_vaapi_options[] = {
    ...
    {"eof_action", "Action to take when encountering EOF from secondary input ", OFFSET(fs.opt_eof_action), AV_OPT_TYPE_INT, {.i64 = EOF_ACTION_REPEAT}, EOF_ACTION_REPEAT, EOF_ACTION_PASS, .flags = FLAGS, .unit = "eof_action"},
