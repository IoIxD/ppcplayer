...
static int config_input(AVFilterLink *inlink)
{
    ...
    V(FRAME_RATE) = V(FR) =
        inlink->frame_rate.num && inlink->frame_rate.den ? av_q2d(inlink->frame_rate) : NAN;
...
static inline char *double2int64str(char *buf, double v)
{
    if (isnan(v))
        ...
        snprintf(buf, BUF_SIZE, "%lld", (int64_t)v);
    return buf;
}
...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        av_log(inlink->dst, AV_LOG_TRACE, " INTERLACED:%lld",
               (int64_t)V(INTERLACED));
        break;
    case AVMEDIA_TYPE_AUDIO:
        av_log(inlink->dst, AV_LOG_TRACE, " NB_SAMPLES:%lld NB_CONSUMED_SAMPLES:%lld",
