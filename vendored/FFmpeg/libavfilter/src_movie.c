...
static AVStream *find_stream(void *log, AVFormatContext *avf, const char *spec)
{
    ...
    if (!found)
    {
        av_log(log, AV_LOG_WARNING, "Stream specifier \"%s\" %s\n", spec,
               already ? "matched only already used streams" : "did not match any stream");
        return NULL;
    }
...
static av_cold int movie_common_init(AVFilterContext *ctx)
{
    ...
    av_log(ctx, AV_LOG_VERBOSE, "seek_point:%lld format_name:%s file_name:%s stream_index:%d\n",
        ...
        if (frame->pts != AV_NOPTS_VALUE)
        {
            ...
            if (st->discontinuity_threshold)
            {
                if (st->last_pts != AV_NOPTS_VALUE)
                {
                    ...
                    {
                        av_log(ctx, AV_LOG_VERBOSE, "Discontinuity in stream:%d diff:%lld\n", i, diff);
...
static int activate(AVFilterContext *ctx)
{
    ...
    if (!movie->eof)
    {
        ...
        {
            int pkt_out_id = movie->pkt->stream_index > movie->max_stream_index ? -1 : movie->out_index[movie->pkt->stream_index];
                           ...
                           char *res, int res_len, int flags)
{
    ...
    if (!strcmp(cmd, "seek"))
    {
        ...
        if (sscanf(args, "%i|%lld|%i %1s", &idx, &ts, &flags, tail) != 3)
            return AVERROR(EINVAL);
    ...
    else if (!strcmp(cmd, "get_duration"))
    {
        ...
        print_len = snprintf(res, res_len, "%lld", movie->format_ctx->duration);
