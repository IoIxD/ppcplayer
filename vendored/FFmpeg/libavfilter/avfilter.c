...
#ifdef TRACE
    ff_tlog(ctx,
            "ref[%p buf:%p data:%p linesize[%d, %d, %d, %d] pts:%lld",
    ...
    if (ref->width)
    {
        ff_tlog(ctx, " a:%d/%d s:%dx%d i:%c iskey:%d type:%c",
                ...
                !(ref->flags & AV_FRAME_FLAG_INTERLACED) ? 'P' : /* Progressive  */
                    (ref->flags & AV_FRAME_FLAG_TOP_FIELD_FIRST) ? 'T'
                                                                 : 'B', /* Top / Bottom */
...
static int ff_request_frame_to_filter(AVFilterLink *link)
{
    ...
    FF_TPRINTF_START(NULL, request_frame_to_filter);
    ff_tlog_link(NULL, link, 1);
...
static const AVOption avfilter_options[] = {
    {"thread_type", "Allowed thread types", OFFSET(thread_type), AV_OPT_TYPE_FLAGS, {.i64 = AVFILTER_THREAD_SLICE}, 0, INT_MAX, FLAGS, .unit = "thread_type"},
    {"slice", NULL, 0, AV_OPT_TYPE_CONST, {.i64 = AVFILTER_THREAD_SLICE}, .flags = FLAGS, .unit = "thread_type"},
    {"enable", "set enable expression", OFFSET(enable_str), AV_OPT_TYPE_STRING, {.str = NULL}, .flags = TFLAGS},
    {"threads", "Allowed number of threads", OFFSET(nb_threads), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, FLAGS, .unit = "threads"},
    {"auto", "autodetect a suitable number of threads to use", 0, AV_OPT_TYPE_CONST, {.i64 = 0}, .flags = FLAGS, .unit = "threads"},
    {"extra_hw_frames", "Number of extra hardware frames to allocate for the user", OFFSET(extra_hw_frames), AV_OPT_TYPE_INT, {.i64 = -1}, -1, INT_MAX, FLAGS},
    {NULL},
};
...
static int ff_filter_frame_to_filter(AVFilterLink *link)
{
    ...
    ret = link->min_samples ? ff_inlink_consume_samples(link, link->min_samples, link->max_samples, &frame) : ff_inlink_consume_frame(link, &frame);
    av_assert1(ret);
    if (ret < 0)
    {
        av_assert1(!frame);
...
int ff_filter_activate(AVFilterContext *filter)
{
    ...
    filter->ready = 0;
    ret = filter->filter->activate ? filter->filter->activate(filter) : ff_filter_activate_default(filter);
