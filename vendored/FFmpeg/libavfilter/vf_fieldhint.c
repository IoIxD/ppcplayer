...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    while (1)
    {
        if (fgets(buf, sizeof(buf) - 1, s->hint))
        {
            ...
            else if (sscanf(buf, "%lld,%lld %c %c", &tf, &bf, &hint, &field) == 4)
            {
                ;
            }
            else if (sscanf(buf, "%lld,%lld %c", &tf, &bf, &hint) == 3)
            {
                ;
            }
            else if (sscanf(buf, "%lld,%lld", &tf, &bf) == 2)
            {
                ;
            }
            else
            {
                av_log(ctx, AV_LOG_ERROR, "Invalid entry at line %lld.\n", s->line);
            ...
            case ABSOLUTE_HINT:
                ...
                {
                    av_log(ctx, AV_LOG_ERROR, "Out of range frames %lld and/or %lld on line %lld for %lld. input frame.\n", tf, bf, s->line, inlink->frame_count_out);
                    return AVERROR_INVALIDDATA;
                }
                break;
            case PATTERN_HINT:
            case RELATIVE_HINT:
                if (tf > 1 || tf < -1 ||
                    bf > 1 || bf < -1)
                {
                    av_log(ctx, AV_LOG_ERROR, "Out of range %lld and/or %lld on line %lld for %lld. input frame.\n", tf, bf, s->line, inlink->frame_count_out);
                    return AVERROR_INVALIDDATA;
                }
                break;
            default:
        ...
        else
        {
            ...
            av_log(ctx, AV_LOG_ERROR, "Missing entry for %lld. input frame.\n", inlink->frame_count_out);
            return AVERROR_INVALIDDATA;
        }
    }
