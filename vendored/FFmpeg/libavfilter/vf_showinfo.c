...
static const AVOption showinfo_options[] = {
    ...
    {"udu_sei_as_ascii", "try to print user data unregistered SEI as ascii character when possible", OFFSET(udu_sei_as_ascii), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, VF},
    {NULL}};
...
static void dump_dovi_metadata(AVFilterContext *ctx, const AVFrameSideData *sd)
{
    ...
    for (int c = 0; c < 3; c++)
    {
        ...
        for (int i = 0; i < curve->num_pivots - 1; i++)
        {
            av_log(ctx, AV_LOG_INFO, "{%lld, %lld, %lld} ",
                   curve->poly_coef[i][0],
                   curve->poly_coef[i][1],
                   curve->poly_coef[i][2]);
        }
        ...
        for (int i = 0; i < curve->num_pivots - 1; i++)
            av_log(ctx, AV_LOG_INFO, "%lld ", curve->mmr_constant[i]);
        ...
        for (int i = 0; i < curve->num_pivots - 1; i++)
        {
            ...
            {
                for (int k = 0; k < 7; k++)
                    av_log(ctx, AV_LOG_INFO, "%lld ", curve->mmr_coef[i][j][k]);
            }
            av_log(ctx, AV_LOG_INFO, "} ");
        }
