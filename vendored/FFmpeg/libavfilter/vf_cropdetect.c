...
static int checkline(void *ctx, const unsigned char *src, int stride, int len, int bpp)
{
    ...
    case 1:
        while (len >= 8)
        {
            total += src[0] + src[stride] + src[2 * stride] + src[3 * stride] + src[4 * stride] + src[5 * stride] + src[6 * stride] + src[7 * stride];
            src += 8 * stride;
            len -= 8;
        }
        while (--len >= 0)
    ...
    case 2:
        ...
        while (len >= 8)
        {
            total += src16[0] + src16[stride] + src16[2 * stride] + src16[3 * stride] + src16[4 * stride] + src16[5 * stride] + src16[6 * stride] + src16[7 * stride];
            src16 += 8 * stride;
            len -= 8;
        }
        while (--len >= 0)
    ...
    case 4:
        while (len >= 4)
        {
            total += src[0] + src[1] + src[2] + src[stride] + src[1 + stride] + src[2 + stride] + src[2 * stride] + src[1 + 2 * stride] + src[2 + 2 * stride] + src[3 * stride] + src[1 + 3 * stride] + src[2 + 3 * stride];
            src += 4 * stride;
            len -= 4;
        }
        while (--len >= 0)
    ...
    {                                                                                     \
        ...
        { // MODE_MV_EDGES
            ...
            else
            {
                ...
                // assert x1<x2, y1<y2
                if (s->x1 > s->x2)
                    FFSWAP(int, s->x1, s->x2);
                if (s->y1 > s->y2)
                    FFSWAP(int, s->y1, s->y2);
...
#define FIND_EDGE(DST, FROM, NOEND, INC, STEP0, STEP1, LEN)           \
    ...
    {                                                                 \
        ...
        av_log(ctx, AV_LOG_INFO,
               "x1:%d x2:%d y1:%d y2:%d w:%d h:%d x:%d y:%d pts:%lld t:%f limit:%f crop=%d:%d:%d:%d\n",
