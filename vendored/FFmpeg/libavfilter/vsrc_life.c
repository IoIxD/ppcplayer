...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    av_log(ctx, AV_LOG_VERBOSE,
           "s:%dx%d r:%d/%d rule:%s stay_rule:%d born_rule:%d stitch:%d seed:%lld\n",
...
static void evolve(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < life->h; i++)
    {
        for (j = 0; j < life->w; j++)
        {
            ...
            if (alive)
                *newbuf = ALIVE_CELL; // new cell is alive
            else if (cell)
                *newbuf = cell - 1; // new cell is dead and in the process of mold
            else
                *newbuf = 0; // new cell is definitely dead
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    if (life->mold || memcmp(life->life_color, "\xff\xff\xff", 3) || memcmp(life->death_color, "\x00\x00\x00", 3))
    {
