...
static int set_control(AVFilterContext *ctx, unsigned long port, LADSPA_Data value)
{
    ...
    {
        av_log(ctx, AV_LOG_ERROR, "Control c%lld is out of range [0 - %lu].\n",
        ...
        value < h->LowerBound)
    {
        av_log(ctx, AV_LOG_ERROR,
               "%s: input control c%lld is below lower boundary of %0.4f.\n",
    ...
    {
        av_log(ctx, AV_LOG_ERROR,
               "%s: input control c%lld is above upper boundary of %0.4f.\n",
                           ...
                           char *res, int res_len, int flags)
{
    ...
    if (av_sscanf(cmd, "c%lld", &port) + av_sscanf(args, "%f", &value) != 2)
        return AVERROR(EINVAL);
