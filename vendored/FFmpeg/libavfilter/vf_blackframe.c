...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    if (pblack >= s->bamount)
    {
        ...
        av_log(ctx, AV_LOG_INFO, "frame:%u pblack:%u pts:%lld t:%f "
...
static const AVOption blackframe_options[] = {
    {"amount", "percentage of the pixels that have to be below the threshold "
     ...
     100,
     FLAGS},
    {"threshold", "threshold below which a pixel value is considered black", OFFSET(bthresh), AV_OPT_TYPE_INT, {.i64 = 32}, 0, 255, FLAGS},
    {"thresh", "threshold below which a pixel value is considered black", OFFSET(bthresh), AV_OPT_TYPE_INT, {.i64 = 32}, 0, 255, FLAGS},
    {NULL}};
