                      ...
                      size_t frames)
{
    ...
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | in: %lld", l->frame_count_in);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
    if ((flags & FLAG_FCOUT) && (!(mode & MODE_NOZERO) || l->frame_count_out))
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | out: %lld", l->frame_count_out);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
    if ((flags & FLAG_FC_DELTA) && (!(mode & MODE_NOZERO) || (l->frame_count_in - l->frame_count_out)))
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | delta: %lld", l->frame_count_in - l->frame_count_out);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
    if ((flags & FLAG_SCIN) && (!(mode & MODE_NOZERO) || l->sample_count_in))
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | sin: %lld", l->sample_count_in);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
    if ((flags & FLAG_SCOUT) && (!(mode & MODE_NOZERO) || l->sample_count_out))
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | sout: %lld", l->sample_count_out);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
    if ((flags & FLAG_SC_DELTA) && (!(mode & MODE_NOZERO) || (l->sample_count_in - l->sample_count_out)))
    {
        len = snprintf(buffer, sizeof(buffer) - 1, " | sdelta: %lld", l->sample_count_in - l->sample_count_out);
        drawtext(out, xpos, ypos, buffer, len, s->white);
        xpos += len * 8;
    }
