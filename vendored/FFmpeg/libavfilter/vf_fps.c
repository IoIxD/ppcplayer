...
static AVFrame *shift_frame(AVFilterContext *ctx, FPSContext *s)
{
    ...
    {
        av_log(ctx, AV_LOG_DEBUG, "Duplicated frame with pts %lld %d times\n",
               frame->pts, s->cur_frame_out - 1);
        s->dup += s->cur_frame_out - 1;
    }
    else if (s->cur_frame_out == 0)
    {
        av_log(ctx, AV_LOG_DEBUG, "Dropping frame with pts %lld\n",
               frame->pts);
        s->drop++;
    }
    s->cur_frame_out = 0;
...
static int config_props(AVFilterLink *outlink)
{
    ...
    if (s->start_time != DBL_MAX && s->start_time != AV_NOPTS_VALUE)
    {
        ...
        av_log(ctx, AV_LOG_VERBOSE, "Set first pts to (in:%lld out:%lld) from start time %f\n",
...
static int read_frame(AVFilterContext *ctx, FPSContext *s, AVFilterLink *inlink, AVFilterLink *outlink)
{
    ...
    av_log(ctx, AV_LOG_DEBUG, "Read frame with in pts %lld, out pts %lld\n",
           in_pts, frame->pts);
...
static int write_frame(AVFilterContext *ctx, FPSContext *s, AVFilterLink *outlink, int *again)
{
    ...
    if (s->next_pts == AV_NOPTS_VALUE)
    {
        if (s->frames[0]->pts != AV_NOPTS_VALUE)
        {
            ...
            av_log(ctx, AV_LOG_VERBOSE, "Set first pts to %lld\n", s->next_pts);
        }
        else
        {
    ...
    else
    {
        ...
        av_log(ctx, AV_LOG_DEBUG, "Writing frame with pts %lld to pts %lld\n",
...
static void update_eof_pts(AVFilterContext *ctx, FPSContext *s, AVFilterLink *inlink, AVFilterLink *outlink, int64_t status_pts)
{
    ...
    av_log(ctx, AV_LOG_DEBUG, "EOF is at pts %lld\n", s->status_pts);
}
