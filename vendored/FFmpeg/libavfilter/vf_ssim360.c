...
static const AVOption ssim360_options[] = {
    ...
    {"ref_projection", "projection of the reference video", OFFSET(ref_projection), AV_OPT_TYPE_INT, {.i64 = PROJECTION_EQUIRECT}, 0, PROJECTION_N - 1, .flags = FLAGS, .unit = "projection"},
    ...
    {"main_projection", "projection of the main video", OFFSET(main_projection), AV_OPT_TYPE_INT, {.i64 = PROJECTION_N}, 0, PROJECTION_N, .flags = FLAGS, .unit = "projection"},

    {"ref_stereo", "stereo format of the reference video", OFFSET(ref_stereo_format), AV_OPT_TYPE_INT, {.i64 = STEREO_FORMAT_MONO}, 0, STEREO_FORMAT_N - 1, .flags = FLAGS, .unit = "stereo_format"},
    ...
    {"main_stereo", "stereo format of main video", OFFSET(main_stereo_format), AV_OPT_TYPE_INT, {.i64 = STEREO_FORMAT_N}, 0, STEREO_FORMAT_N, .flags = FLAGS, .unit = "stereo_format"},
             ...
             float angular_resolution, HeatmapList *heatmaps)
{
    ...
    for (y = 1; y < vertical_block_count; y++)
    {
        ...
        if (max_value > 255)
        {
            ...
            sample_ssim360 = (2. * fs1 * fs2 + ssim_c1_d) * (2. * covar + ssim_c2_d) / ((1. * fs1 * fs1 + 1. * fs2 * fs2 + ssim_c1_d) * (1. * vars + ssim_c2_d));
        }
        else
        {
            static const int ssim_c1 = (int)(.01 * .01 * 255 * 255 * 64 + .5);
            ...
            int vars = fss * 64 - fs1 * fs1 - fs2 * fs2;
            int covar = fs12 * 64 - fs1 * fs2;
            sample_ssim360 = (double)(2 * fs1 * fs2 + ssim_c1) * (double)(2 * covar + ssim_c2) / ((double)(fs1 * fs1 + fs2 * fs2 + ssim_c1) * (double)(vars + ssim_c2));
        }
...
static int do_ssim360(FFFrameSync *fs)
{
    ...
    if (s->use_tape)
    {
        ...
        if (s->stats_file)
        {
            fprintf(s->stats_file, "n:%lld ", s->nb_ssim_frames);
