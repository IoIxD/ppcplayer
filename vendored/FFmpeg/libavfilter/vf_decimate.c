...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    if (av_log_get_level() >= AV_LOG_DEBUG)
    {
        ...
        {
            av_log(ctx, AV_LOG_DEBUG, "  #%d: totdiff=%08lld maxbdiff=%08lld%s%s%s%s\n",
    ...
    for (i = 0; i < dm->cycle && dm->queue[i].frame; i++)
    {
        ...
        else
        {
            ...
            frame->pts = dm->last_duration ? dm->last_pts + dm->last_duration : (dm->start_pts == AV_NOPTS_VALUE ? 0 : dm->start_pts);
