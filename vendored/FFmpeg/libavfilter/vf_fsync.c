...
static int activate(AVFilterContext *ctx)
{
    ...
    if (s->last_frame)
    {
        ret = av_sscanf(s->cur, "%lld %lld %d/%d", &s->ptsi, &s->pts, &s->tb_num, &s->tb_den);
        if (ret != 4)
        {
        ...
        av_log(ctx, AV_LOG_DEBUG, "frame %lld ", s->last_frame->pts);

        if (s->last_frame->pts >= s->ptsi)
        {
            av_log(ctx, AV_LOG_DEBUG, ">= %lld: DUP LAST with pts = %lld\n", s->ptsi, s->pts);
        ...
        {
            av_log(ctx, AV_LOG_DEBUG, "<  %lld: DROP\n", s->ptsi);
