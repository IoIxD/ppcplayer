...
#define FILTER(xm2, xm1, xp1, xp2) \
    abs(4 * srcp[x] - 3 * (srcp[x + (xm1) * src_linesize] + srcp[x + (xp1) * src_linesize]) + (srcp[x + (xm2) * src_linesize] + srcp[x + (xp2) * src_linesize])) > cthresh6
...
{
    if (match == mP || match == mB)
        return fm->prv;
    else if (match == mN || match == mU)
        return fm->nxt;
    else /* match == mC */
        return fm->src;
}
                                   ...
                                   const AVFrame *prv, AVFrame *src, const AVFrame *nxt, int input)
{
    ...
    else
    {
        ...
        {
        case mP:
            copy_fields(fm, dst, src, 1 - field, input);
            copy_fields(fm, dst, prv, field, input);
            break;
        case mN:
            copy_fields(fm, dst, src, 1 - field, input);
            copy_fields(fm, dst, nxt, field, input);
            break;
        case mB:
            copy_fields(fm, dst, src, field, input);
            copy_fields(fm, dst, prv, 1 - field, input);
            break;
        case mU:
            copy_fields(fm, dst, src, field, input);
            copy_fields(fm, dst, nxt, 1 - field, input);
            break;
        default:
            av_assert0(0);
        }
...
#define SLIDING_FRAME_WINDOW(prv, src, nxt)                                                        \
    ...
    /* parity */
    order = fm->order != FM_PARITY_AUTO ? fm->order : ((in->flags & AV_FRAME_FLAG_INTERLACED) ? !!(in->flags & AV_FRAME_FLAG_TOP_FIELD_FIRST) : 1);
    ...
    if (interlaced_frame)
    {
        ...
        av_log(ctx, AV_LOG_WARNING, "Frame #%lld at %s is still interlaced\n",
