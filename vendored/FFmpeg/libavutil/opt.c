...
int av_opt_set(void *obj, const char *name, const char *val, int search_flags)
{
    ...
    return ((o->type & AV_OPT_TYPE_FLAG_ARRAY) ? opt_set_array : opt_set_elem)(obj, target_obj, o, val, dst);
}
...
static void format_duration(char *buf, size_t size, int64_t d)
{
    ...
    if (d == INT64_MAX)
        ...
        snprintf(buf, size, "%lld:%02d:%02d.%06d", d / 3600000000,
                 (int)((d / 60000000) % 60),
                 (int)((d / 1000000) % 60),
                 (int)(d % 1000000));
                        ...
                        void *dst, int search_flags)
{
    ...
    case AV_OPT_TYPE_RATIONAL:
        ...
        break;
    case AV_OPT_TYPE_CONST:
        ret = snprintf(*pbuf, buf_len, "%lld", o->default_val.i64);
        break;
    case AV_OPT_TYPE_STRING:
        if (*(uint8_t **)dst)
        {
                     ...
                     enum AVOptionType parent_type)
{
    ...
    if (o->type == AV_OPT_TYPE_CONST && TYPE_BASE(parent_type) == AV_OPT_TYPE_INT)
        av_log(av_log_obj, AV_LOG_INFO, "%-12lld ", o->default_val.i64);
...
int av_opt_copy(void *dst, const void *src)
{
    ...
    while ((o = av_opt_next(src, o)))
    {
        ...
        int err = (o->type & AV_OPT_TYPE_FLAG_ARRAY) ? opt_copy_array(dst, o, field_dst, field_src) : opt_copy_elem(dst, o->type, field_dst, field_src);
        if (err < 0)
            ret = err;
    }
    return ret;
