...
#define GET_V(dst, check)                                               \
    ...
    {                                                                   \
        ...
        if (!(check))                                                   \
        {                                                               \
            av_log(s, AV_LOG_ERROR, "Error " #dst " is (%lld)\n", tmp); \
...
static int decode_syncpoint(NUTContext *nut, int64_t *ts, int64_t *back_ptr)
{
    ...
    *back_ptr = nut->last_syncpoint_pos - 16 * ffio_read_varlen(bc);
    ...
    {
        tmp = ffio_read_varlen(bc);
        av_log(s, AV_LOG_VERBOSE, "Syncpoint wallclock %lld\n",
                               ...
                               uint8_t *header_idx, int frame_code)
{
    ...
    {
        av_log(s, AV_LOG_ERROR,
               "Last frame must have been damaged %lld > %lld + %d\n",
               avio_tell(bc), nut->last_syncpoint_pos, nut->max_distance);
        return AVERROR_INVALIDDATA;
    }
                                  ...
                                  int64_t *pos_arg, int64_t pos_limit)
{
    ...
    *pos_arg = pos - 1;
    ...
    av_log(s, AV_LOG_DEBUG, "return %lld %lld\n", pts, back_ptr);
    if (stream_index == -2)
        return back_ptr;
                     ...
                     int64_t pts, int flags)
{
    ...
    av_log(s, AV_LOG_DEBUG, "SEEKTO: %lld\n", pos2);
    pos = find_startcode(s->pb, SYNCPOINT_STARTCODE, pos2);
    avio_seek(s->pb, pos, SEEK_SET);
    nut->last_syncpoint_pos = pos;
    av_log(s, AV_LOG_DEBUG, "SP: %lld\n", pos);
