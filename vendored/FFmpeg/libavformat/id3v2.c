...
static const ID3v2EMFunc *get_extra_meta_func(const char *tag, int isv34)
{
    ...
    {
        if (tag && !memcmp(tag,
                           (isv34 ? id3v2_extra_meta_funcs[i].tag4 : id3v2_extra_meta_funcs[i].tag3),
                           (isv34 ? 4 : 3)))
    ...
    while (len >= taghdrlen)
    {
                  ...
                  (extra_func = get_extra_meta_func(tag, isv34))))
        {
            ...
            if (tcomp)
            {
                ...
                av_log(s, AV_LOG_DEBUG, "Compresssed frame %s tlen=%d dlen=%lld\n", tag, tlen, dlen);
                ...
                {
                    av_log(s, AV_LOG_ERROR, "Failed to alloc %lld bytes\n", dlen);
                    goto seek;
                }
