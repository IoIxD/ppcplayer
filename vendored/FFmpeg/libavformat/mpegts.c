...
#define MPEGTS_OPTIONS                                                                                                                                                                               \
    {"resync_size", "set size limit for looking up a new synchronization", offsetof(MpegTSContext, resync_size), AV_OPT_TYPE_INT, {.i64 = MAX_RESYNC_SIZE}, 0, INT_MAX, AV_OPT_FLAG_DECODING_PARAM}, \
        {"ts_id", "transport stream id", offsetof(MpegTSContext, id), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY},                                           \
    {                                                                                                                                                                                                \
...
static int mpegts_read_header(AVFormatContext *s)
{
    ...
    else
    {
        ...
        for (;;)
        {
                ...
                parse_pcr(&pcr_h, &pcr_l, data) == 0)
            {
                ...
                if (nb_pcrs >= 2)
                {
                    ...
                    {
                        av_log(ts->stream, AV_LOG_WARNING, "invalid pcr pair %lld >= %lld\n", pcrs[0], pcrs[1]);
                        pcrs[0] = pcrs[1];
                        nb_pcrs--;
                    }
                }
        ...
        av_log(ts->stream, AV_LOG_TRACE, "start=%0.3f pcr=%0.3f incr=%lld\n",
