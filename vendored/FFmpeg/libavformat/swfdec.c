...
static int swf_probe(const AVProbeData *p)
{
    ...
    /* check file header */
    if (AV_RB24(p->buf) != AV_RB24("CWS") && AV_RB24(p->buf) != AV_RB24("FWS"))
        return 0;

    if (AV_RB24(p->buf) == AV_RB24("CWS") && p->buf[3] <= 20)
...
static int swf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;)
    {
        ...
        else if (tag == TAG_DEFINEBITSLOSSLESS || tag == TAG_DEFINEBITSLOSSLESS2)
        {
            ...
            ff_dlog(s, "bitmap: ch=%d fmt=%d %dx%d (linesize=%d) len=%d->%lld pal=%d\n",
            ...
            case 3:
                ...
                for (i = 0; i < colormapsize; i++)
                    if (alpha_bmp)
                        colormap[i] = buf[3] << 24 | AV_RB24(buf + 4 * i);
                    else
                        colormap[i] = 0xffU << 24 | AV_RB24(buf + 3 * i);
