...
static int rtsp_read_announce(AVFormatContext *s)
{
    ...
    if (request.content_length)
    {
        ...
        /* Read SDP */
        if (ffurl_read_complete(rt->rtsp_hd, sdp, request.content_length) < request.content_length)
        {
...
static int rtsp_listen(AVFormatContext *s)
{
    ...
    for (;;)
    { /* Wait for incoming RTSP messages */
        ret = read_line(s, rbuf, sizeof(rbuf), &rbuflen);
        if (ret < 0)
            goto fail;
        av_log(s, AV_LOG_TRACE, "Parsing[%d]: %s\n", rbuflen, rbuf);
...
static int rtsp_read_header(AVFormatContext *s)
{
    ...
    else
    {
        ...
        rt->real_setup_cache = !s->nb_streams ? NULL : av_calloc(s->nb_streams, 2 * sizeof(*rt->real_setup_cache));
