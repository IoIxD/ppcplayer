...
static const AVOption tedcaptions_options[] = {
    {"start_time", "set the start time (offset) of the subtitles, in ms", offsetof(TEDCaptionsDemuxer, start_time), AV_OPT_TYPE_INT64, {.i64 = 15000}, INT64_MIN, INT64_MAX, AV_OPT_FLAG_SUBTITLE_PARAM | AV_OPT_FLAG_DECODING_PARAM},
    {NULL},
};
...
static void next_byte(AVIOContext *pb, int *cur_byte)
{
    ...
    *cur_byte = ret > 0 ? b : ret == 0 ? AVERROR_EOF
                                       : ret;
}
...
static av_cold int tedcaptions_read_header(AVFormatContext *avf)
{
    ...
    if (ret < 0)
    {
        if (ret == AVERROR_INVALIDDATA)
            av_log(avf, AV_LOG_ERROR, "Syntax error near offset %lld.\n",
                   avio_tell(avf->pb));
        return ret;
    }
...
static av_cold int tedcaptions_read_probe(const AVProbeData *p)
{
    ...
    return count == FF_ARRAY_ELEMS(tags) ? AVPROBE_SCORE_MAX : count ? AVPROBE_SCORE_EXTENSION
                                                                     : 0;
}
