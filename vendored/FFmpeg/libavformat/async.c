...
static int64_t async_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    {
        av_log(h, AV_LOG_TRACE, "async_seek: AVSEEK_SIZE: %lld\n", (int64_t)c->logical_size);
        return c->logical_size;
    }
    else if (whence == SEEK_CUR)
    {
        av_log(h, AV_LOG_TRACE, "async_seek: %lld\n", pos);
             ...
             (new_logical_pos < (c->logical_pos + fifo_size + SHORT_SEEK_THRESHOLD)))
    {
        ...
        /* fast seek */
        av_log(h, AV_LOG_TRACE, "async_seek: fask_seek %lld from %d dist:%d/%d\n",
...
int main(void)
{
    ...
    while (1) {
        ...
        if (ret == AVERROR_EOF) {
            printf("read-error: AVERROR_EOF at %lld\n", ffurl_seek(h, 0, SEEK_CUR));
            break;
        }
        else if (ret == 0)
            break;
        else if (ret < 0) {
            printf("read-error: %d at %lld\n", ret, ffurl_seek(h, 0, SEEK_CUR));
            goto fail;
        } else {
            for (i = 0; i < ret; ++i) {
                if (buf[i] != (pos & 0xFF)) {
                    printf("read-mismatch: actual %d, expecting %d, at %lld\n",
