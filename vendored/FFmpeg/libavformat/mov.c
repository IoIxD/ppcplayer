...
retry:
    ...
    else
    {
        ...
        { // BE signed integer, variable size
            int val = 0;
            if (str_size == 1)
        ...
        { // BE unsigned integer, variable size
            unsigned int val = 0;
            if (str_size == 1)
                val = avio_r8(pb);
            else if (str_size == 2)
...
#define DDTS_SIZE 20
    ...
    st->codecpar->frame_size =
        (frame_duration_code == 0) ? 512 : (frame_duration_code == 1) ? 1024
                                       : (frame_duration_code == 2)   ? 2048
                                       : (frame_duration_code == 3)   ? 4096
                                                                      : 0;
...
static int mov_read_chnl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (avio_tell(pb) != end)
    {
        av_log(c->fc, AV_LOG_WARNING, "skip %lld bytes of unknown data inside chnl\n",
               end - avio_tell(pb));
...
static int mov_read_aclr(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->fc->nb_streams >= 1)
    {
        ...
        {
            av_log(c->fc, AV_LOG_WARNING, "aclr not decoded - unexpected size %lld\n", atom.size);
        }
    }
...
static int mov_read_wave(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    { /* to read frma, esds atoms */
        if (st->codecpar->codec_id == AV_CODEC_ID_ALAC && atom.size >= 24)
        {
            ...
            buffer = avio_rb64(pb);
            atom.size -= 8;
            if ((buffer & 0xFFFFFFFF) == MKBETAG('f', 'r', 'm', 'a') && buffer >> 32 <= atom.size && buffer >> 32 >= 8)
            {
                avio_skip(pb, -8);
                atom.size += 8;
            }
                                 ...
                                 AVStream *st, MOVStreamContext *sc)
{
    ...
    case AV_CODEC_ID_PCM_S16BE:
        if (st->codecpar->bits_per_coded_sample == 8)
            ...
            st->codecpar->codec_id =
                st->codecpar->codec_id == AV_CODEC_ID_PCM_S16BE ? AV_CODEC_ID_PCM_S24BE : AV_CODEC_ID_PCM_S24LE;
        else if (st->codecpar->bits_per_coded_sample == 32)
            st->codecpar->codec_id =
                st->codecpar->codec_id == AV_CODEC_ID_PCM_S16BE ? AV_CODEC_ID_PCM_S32BE : AV_CODEC_ID_PCM_S32LE;
        break;
...
static int mov_read_sdtp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_log(c->fc, AV_LOG_TRACE, "track[%u].sdtp.entries = %lld\n",
           c->fc->nb_streams - 1, entries);
                               ...
                               int size, int distance, int flags)
{
    ...
    const size_t requested_size =
        min_size_needed > sti->index_entries_allocated_size ? FFMAX(min_size_needed, 2 * sti->index_entries_allocated_size) : min_size_needed;
                              ...
                              int count, int duration)
{
    ...
    const size_t requested_size =
        min_size_needed > *allocated_size ? FFMAX(min_size_needed, 2 * (*allocated_size)) : min_size_needed;
...
static void mov_fix_index(MOVContext *mov, AVStream *st)
{
                               ...
                               &edit_list_duration, mov->time_scale))
    {
        ...
        for (; current < e_old_end; current++, index++)
        {
            ...
            frame_duration = (current + 1 < e_old_end) ? ((current + 1)->timestamp - current->timestamp) : edit_list_duration;
            ...
            if (ctts_data_old && ctts_index_old < ctts_count_old)
            {
                ...
                av_log(mov->fc, AV_LOG_TRACE, "stts: %lld ctts: %lld, ctts_index: %lld, ctts_count: %lld\n",
            ...
            if (curr_cts < edit_list_media_time || curr_cts >= (edit_list_duration + edit_list_media_time))
            {
                    ...
                    first_non_zero_audio_edit > 0)
                {
                    ...
                    av_log(mov->fc, AV_LOG_DEBUG, "skip %d audio samples from curr_cts: %lld\n", packet_skip_samples, curr_cts);
                }
                else
                {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
    {
        if (msc->min_corrected_pts > 0)
        {
            av_log(mov->fc, AV_LOG_DEBUG, "Offset DTS by %lld to make first pts zero.\n", msc->min_corrected_pts);
          ...
          sc->stts_count == 1 && sc->stts_data[0].duration == 1))
    {
        ...
        for (i = 0; i < sc->chunk_count; i++)
        {
            ...
            for (j = 0; j < sc->stsc_data[stsc_index].count; j++)
            {
                ...
                if (sc->keyframe_absent && !sc->stps_count && !rap_group_present && (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO || (i == 0 && j == 0)))
                    keyframe = 1;
                if (keyframe)
                    distance = 0;
                ...
                {
                    av_log(mov->fc, AV_LOG_ERROR, "Current offset %lld or sample size %u is too large\n",
                           current_offset,
                           sample_size);
                    return;
                }
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_MP3 && sc->stts_count > 3 && sc->stts_count * 10 > st->nb_frames && sc->time_scale == st->codecpar->sample_rate)
    {
        ffstream(st)->need_parsing = AVSTREAM_PARSE_FULL;
    }
...
static int mov_read_tfhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    frag->base_data_offset = flags & MOV_TFHD_BASE_DATA_OFFSET ? avio_rb64(pb) : flags & MOV_TFHD_DEFAULT_BASE_IS_MOOF ? frag->moof_offset
                                                                                                                       : frag->implicit_offset;
    frag->stsd_id = flags & MOV_TFHD_STSD_ID ? avio_rb32(pb) : trex->stsd_id;

    frag->duration = flags & MOV_TFHD_DEFAULT_DURATION ? avio_rb32(pb) : trex->duration;
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (frag_stream_info)
    {
                 ...
                 c->use_mfra_for == FF_MOV_FLAG_MFRA_PTS)
        {
            ...
            av_log(c->fc, AV_LOG_DEBUG, "found mfra time %lld, using it for pts\n", pts);
        }
        else if (frag_stream_info->first_tfra_pts != AV_NOPTS_VALUE &&
                 c->use_mfra_for == FF_MOV_FLAG_MFRA_DTS)
        {
            dts = frag_stream_info->first_tfra_pts;
            av_log(c->fc, AV_LOG_DEBUG, "found mfra time %lld, using it for dts\n", pts);
        }
        else
        {
            ...
            if (has_tfdt && c->use_tfdt || fallback_tfdt)
            {
                ...
                av_log(c->fc, AV_LOG_DEBUG, "found tfdt time %lld, using it for dts\n", dts);
            }
            else if (has_sidx && !c->use_tfdt || fallback_sidx)
            {
                ...
                av_log(c->fc, AV_LOG_DEBUG, "found sidx time %lld, using it for dts\n", frag_stream_info->sidx_pts);
            }
            else
            {
                dts = sc->track_end - sc->time_offset;
                av_log(c->fc, AV_LOG_DEBUG, "found track end time %lld, using it for dts\n", dts);
            }
        }
    }
    else
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (atom.size != edit_count * elst_entry_size)
    {
        if (c->fc->strict_std_compliance >= FF_COMPLIANCE_STRICT)
        {
            av_log(c->fc, AV_LOG_ERROR, "Invalid edit list entry_count: %d for elst atom of size: %lld bytes.\n",
        ...
        else
        {
            ...
            {
                av_log(c->fc, AV_LOG_WARNING, "ELST atom of %lld bytes, bigger than %d entries.\n", atom.size, edit_count);
            }
        }
    }
    ...
    for (i = 0; i < edit_count && atom.size > 0 && !pb->eof_reached; i++)
    {
        ...
        atom.size -= 4;
        av_log(c->fc, AV_LOG_TRACE, "duration=%lld time=%lld rate=%f\n",
               e->duration, e->time, e->rate);

        if (e->time < 0 && e->time != -1 &&
            c->fc->strict_std_compliance >= FF_COMPLIANCE_STRICT)
        {
            av_log(c->fc, AV_LOG_ERROR, "Track %d, edit %d: Invalid edit list media time=%lld\n",
...
static int mov_read_free(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!c->found_moov && !c->found_mdat && !memcmp(content, "Anevia\x1A\x1A", 8) && c->use_mfra_for == FF_MOV_FLAG_MFRA_AUTO)
    {
        c->use_mfra_for = FF_MOV_FLAG_MFRA_PTS;
    }
    ...
    for (int i = 0; i < item_count; i++)
    {
        ...
        for (int j = 0; j < extent_count; j++)
        {
            ...
            av_log(c->fc, AV_LOG_TRACE, "iloc: item_idx %d, offset_type %d, "
                                        "extent_offset %lld, extent_length %lld\n",
...
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    while (total_size <= atom.size - 8)
    {
        ...
        av_log(c->fc, AV_LOG_TRACE, "type:'%s' parent:'%s' sz: %lld %lld %lld\n",
        ...
        else
        {
            ...
            {
                av_log(c->fc, AV_LOG_WARNING,
                       "overread end of atom '%s' by %lld bytes\n",
    ...
    for (;;)
    {
        ...
        case MKTAG('f', 't', 'y', 'p'):
            if (tag == MKTAG('f', 't', 'y', 'p') &&
                (AV_RL32(p->buf + offset + 8) == MKTAG('j', 'p', '2', ' ') || AV_RL32(p->buf + offset + 8) == MKTAG('j', 'p', 'x', ' ') || AV_RL32(p->buf + offset + 8) == MKTAG('j', 'x', 'l', ' ')))
            {
                score = FFMAX(score, 5);
            }
            else
...
static int mov_read_timecode_track(AVFormatContext *s, AVStream *st)
{
    ...
    if (sc->tmcd_flags & 0x0001)
        flags |= AV_TIMECODE_FLAG_DROPFRAME;
    if (sc->tmcd_flags & 0x0002)
        flags |= AV_TIMECODE_FLAG_24HOURSMAX;
    if (sc->tmcd_flags & 0x0004)
        flags |= AV_TIMECODE_FLAG_ALLOWNEGATIVE;
...
static int mov_read_header(AVFormatContext *s)
{
    ...
    av_log(mov->fc, AV_LOG_TRACE, "on_parse_exit_offset=%lld\n", avio_tell(pb));
    ...
    if (mov->trex_data || mov->use_mfra_for > 0)
    {
        for (i = 0; i < s->nb_streams; i++)
        {
            ...
            if (sc->duration_for_fps > 0)
            {
                ...
                {
                    av_log(s, AV_LOG_WARNING, "Overflow during bit rate calculation %lld * 8 * %d\n",
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < s->nb_streams; i++)
    {
        ...
        if (msc->pb && msc->current_sample < avsti->nb_index_entries)
        {
            ...
            av_log(s, AV_LOG_TRACE, "stream %d, sample %d, dts %lld\n", i, msc->current_sample, dts);
                               ...
                               int64_t current_index, AVPacket *pkt)
{
    ...
    {
        int64_t next_dts = (sc->current_sample < ffstream(st)->nb_index_entries) ? ffstream(st)->index_entries[sc->current_sample].timestamp : st->duration;
...
static int mov_seek_stream(AVFormatContext *s, AVStream *st, int64_t timestamp, int flags)
{
    ...
    for (;;)
    {
        ...
        av_log(s, AV_LOG_TRACE, "stream %d, timestamp %lld, sample %d\n", st->index, timestamp, sample);
...
static const AVOption mov_options[] = {
    ...
    {"seek_streams_individually",
     ...
     1,
     FLAGS},
    {"ignore_editlist", "Ignore the edit list atom.", OFFSET(ignore_editlist), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS},
    {"advanced_editlist",
     ...
     1,
     FLAGS},
    {"ignore_chapters", "", OFFSET(ignore_chapters), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS},
    {"use_mfra_for",
     ...
     FLAGS,
     .unit = "use_mfra_for"},
    {"auto", "auto", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_AUTO}, 0, 0, FLAGS, .unit = "use_mfra_for"},
    {"dts", "dts", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_DTS}, 0, 0, FLAGS, .unit = "use_mfra_for"},
    {"pts", "pts", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_PTS}, 0, 0, FLAGS, .unit = "use_mfra_for"},
    {"use_tfdt", "use tfdt for fragment timestamps", OFFSET(use_tfdt), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, FLAGS},
    {"export_all", "Export unrecognized metadata entries", OFFSET(export_all), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, .flags = FLAGS},
    {"export_xmp", "Export full XMP metadata", OFFSET(export_xmp), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, .flags = FLAGS},
    ...
    {"enable_drefs", "Enable external track support.", OFFSET(enable_drefs), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS},
