...
{
    const AVPacketSideData *sd = track->st ? av_packet_side_data_get(track->st->codecpar->coded_side_data,
...
static int mov_write_audio_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    else
    {
        ...
        { /* reserved for mp4/3gp */
            avio_wb16(pb, track->tag == MKTAG('i', 'a', 'm', 'f') ? 0 : track->par->ch_layout.nb_channels);
        ...
        if (track->tag == MKTAG('i', 'a', 'm', 'f'))
            ...
            avio_wb16(pb, track->par->sample_rate <= UINT16_MAX ? track->par->sample_rate : 0);
    ...
    if (track->mode == MODE_MP4 && track->par->codec_type == AVMEDIA_TYPE_AUDIO && ((ret = mov_write_SA3D_tag(s, pb, track)) < 0))
    {
        return ret;
    }

    if (track->mode == MODE_MOV && track->par->codec_type == AVMEDIA_TYPE_AUDIO && ((ret = mov_write_chan_tag(s, pb, track)) < 0))
    {
        return ret;
    }

    if (mov->encryption_scheme != MOV_ENC_NONE && ((ret = ff_mov_cenc_write_sinf_tag(track, pb, mov->encryption_kid)) < 0))
    {
        return ret;
    }
...
static int mov_write_avid_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if (track->par->sample_aspect_ratio.num > 0 && track->par->sample_aspect_ratio.den > 0)
...
static int mov_get_h264_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    else if (track->par->format == AV_PIX_FMT_YUV422P10)
    {
        ...
        else if (track->par->width == 4096 && track->par->height == 2160 || track->par->width == 3840 && track->par->height == 2160 || track->par->width == 2048 && track->par->height == 1080)
        {
            tag = MKTAG('a', 'i', 'v', 'x');
        }
    }
...
static void find_compressor(char *compressor_name, int len, MOVTrack *track)
{
    ...
    int xdcam_res = (track->par->width == 1280 && track->par->height == 720) || (track->par->width == 1440 && track->par->height == 1080) || (track->par->width == 1920 && track->par->height == 1080);
...
static int mov_write_video_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    int uncompressed_ycbcr = ((track->par->codec_id == AV_CODEC_ID_RAWVIDEO && track->par->format == AV_PIX_FMT_UYVY422) || (track->par->codec_id == AV_CODEC_ID_RAWVIDEO && track->par->format == AV_PIX_FMT_YUYV422) || track->par->codec_id == AV_CODEC_ID_V308 || track->par->codec_id == AV_CODEC_ID_V408 || track->par->codec_id == AV_CODEC_ID_V410 || track->par->codec_id == AV_CODEC_ID_V210);
...
static int mov_write_source_reference_tag(AVIOContext *pb, MOVTrack *track, const char *reel_name)
{
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "reel_name length %ulld is too large\n", str_size);
        avio_wb16(pb, 0);
        return AVERROR(EINVAL);
    }
                                            ...
                                            const char *tag, int *lang)
{
    ...
    *lang = 0;
    ...
    {
        len2 = strlen(t2->key);
        if (len2 == len + 4 && !strcmp(t->value, t2->value) && (l = ff_mov_iso639_to_lang(&t2->key[len2 - 3], 1)) >= 0)
        {
            *lang = l;
            return t;
        }
...
static int mov_write_isml_manifest(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s)
{
    ...
    for (i = 0; i < mov->nb_tracks; i++)
    {
        ...
        else
        {
            ...
            param_write_int(pb, "SamplingRate", track->tag == MKTAG('i', 'a', 'm', 'f') ? 0 : track->par->sample_rate);
...
{
    return entry->flags & MOV_SYNC_SAMPLE ? MOV_FRAG_SAMPLE_FLAG_DEPENDS_NO : (MOV_FRAG_SAMPLE_FLAG_DEPENDS_YES | MOV_FRAG_SAMPLE_FLAG_IS_NON_SYNC);
}
    ...
    if (flags & MOV_TFHD_DEFAULT_FLAGS)
    {
        ...
        if (track->entry > 1)
            ...
            track->default_sample_flags =
                track->par->codec_type == AVMEDIA_TYPE_VIDEO ? (MOV_FRAG_SAMPLE_FLAG_DEPENDS_YES | MOV_FRAG_SAMPLE_FLAG_IS_NON_SYNC) : MOV_FRAG_SAMPLE_FLAG_DEPENDS_NO;
...
static int mov_write_uuidprof_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    int64_t frame_rate = video_st->avg_frame_rate.den ? (video_st->avg_frame_rate.num * 0x10000LL) / video_st->avg_frame_rate.den : 0;
...
static int check_pkt(AVFormatContext *s, MOVTrack *trk, AVPacket *pkt)
{
    ...
    {
        av_log(s, AV_LOG_WARNING, "Packet duration: %lld / dts: %lld is out of range\n",
               duration, pkt->dts);
    ...
    {
        av_log(s, AV_LOG_ERROR, "Application provided duration: %lld is invalid\n", pkt->duration);
        return AVERROR(EINVAL);
    }
    return 0;
}
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (trk->start_dts == AV_NOPTS_VALUE)
    {
        ...
        if (trk->frag_discont)
        {
            ...
            av_log(s, AV_LOG_WARNING,
                   "Track %d starts with a nonzero dts %lld, while the moov "
    ...
    else if (pkt->flags & AV_PKT_FLAG_KEY)
    {
        if (mov->mode == MODE_MOV && par->codec_id == AV_CODEC_ID_MPEG2VIDEO &&
            trk->entry > 0)
        { // force sync sample for the first key frame
...
#define IS_MODE(muxer, config) (CONFIG_##config##_MUXER && !strcmp(#muxer, s->oformat->name))
    if (IS_MODE(3gp, TGP))
        mov->mode = MODE_3GP;
    else if (IS_MODE(3g2, TG2))
        mov->mode = MODE_3GP | MODE_3G2;
    else if (IS_MODE(mov, MOV))
        mov->mode = MODE_MOV;
    else if (IS_MODE(psp, PSP))
        mov->mode = MODE_PSP;
    else if (IS_MODE(ipod, IPOD))
        mov->mode = MODE_IPOD;
    else if (IS_MODE(ismv, ISMV))
        mov->mode = MODE_ISM;
    else if (IS_MODE(f4v, F4V))
        mov->mode = MODE_F4V;
    else if (IS_MODE(avif, AVIF))
        mov->mode = MODE_AVIF;
#undef IS_MODE
    ...
    if (mov->write_tmcd == -1 && (mov->mode == MODE_MOV || mov->mode == MODE_MP4) || mov->write_tmcd == 1)
    {
...
static int mov_write_trailer(AVFormatContext *s)
{
        ...
        mov->flags & FF_MOV_FLAG_HYBRID_FRAGMENTED)
    {
        ...
        else if (mov->reserved_moov_size > 0)
        {
            ...
            if (size < 8)
            {
                av_log(s, AV_LOG_ERROR, "reserved_moov_size is too small, needed %lld additional\n", 8 - size);
                return AVERROR(EINVAL);
            }
            avio_wb32(pb, size);
            ffio_wfourcc(pb, "free");
