...
static int init_muxer(AVFormatContext *s, AVDictionary **options)
{
    ...
    for (unsigned i = 0; i < s->nb_streams; i++)
    {
        ...
        if (of->p.codec_tag)
        {
            if (par->codec_tag && par->codec_id == AV_CODEC_ID_RAWVIDEO && (av_codec_get_tag(of->p.codec_tag, par->codec_id) == 0 || av_codec_get_tag(of->p.codec_tag, par->codec_id) == MKTAG('r', 'a', 'w', ' ')) && !validate_codec_tag(s, st))
            {
    ...
    if (!si->interleave_packet)
        si->interleave_packet = si->nb_interleaved_streams > 1 ? ff_interleave_packet_per_dts : ff_interleave_packet_passthrough;
...
static void guess_pkt_duration(AVFormatContext *s, AVStream *st, AVPacket *pkt)
{
    ...
    {
        av_log(s, AV_LOG_WARNING, "Packet with invalid duration %lld in stream %d\n",
    ...
    if (!(s->oformat->flags & AVFMT_NOTIMESTAMPS))
    {
             ...
             sti->cur_dts > pkt->dts))
        {
            av_log(s, AV_LOG_ERROR,
                   ...
                   "dts to muxer in stream %d: %lld >= %lld\n",
        ...
        if (pkt->pts < pkt->dts)
        {
            av_log(s, AV_LOG_ERROR, "pts %lld < dts %lld in stream %d\n",
