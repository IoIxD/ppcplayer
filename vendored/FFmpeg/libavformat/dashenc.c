...
static const char *get_format_str(SegmentType segment_type)
{
    switch (segment_type)
    {
    case SEGMENT_TYPE_MP4:
        return "mp4";
    case SEGMENT_TYPE_WEBM:
        return "webm";
    }
    return NULL;
}
                                ...
                                int representation_id, int final)
{
    ...
    if (c->use_template)
    {
        ...
        if (!c->use_timeline)
        {
            avio_printf(out, "duration=\"%lld\" ", os->seg_duration);
        ...
        if (c->presentation_time_offset)
            avio_printf(out, " presentationTimeOffset=\"%lld\"", c->presentation_time_offset);
        avio_printf(out, ">\n");
        if (c->use_timeline)
        {
            int64_t cur_time = 0;
            ...
            for (i = start_index; i < os->nb_segments;)
            {
                ...
                {
                    cur_time = seg->time;
                    avio_printf(out, "t=\"%lld\" ", seg->time);
                }
                avio_printf(out, "d=\"%lld\" ", seg->duration);
    ...
    else if (c->single_file)
    {
        ...
        avio_printf(out, "\t\t\t\t<SegmentList timescale=\"%d\" duration=\"%lld\" startNumber=\"%d\">\n", AV_TIME_BASE, FFMIN(os->seg_duration, os->last_duration), start_number);
        avio_printf(out, "\t\t\t\t\t<Initialization range=\"%lld-%lld\" />\n", os->init_start_pos, os->init_start_pos + os->init_range_length - 1);
        for (i = start_index; i < os->nb_segments; i++)
        {
            Segment *seg = os->segments[i];
            avio_printf(out, "\t\t\t\t\t<SegmentURL mediaRange=\"%lld-%lld\" ", seg->start_pos, seg->start_pos + seg->range_length - 1);
            if (seg->index_length)
                avio_printf(out, "indexRange=\"%lld-%lld\" ", seg->start_pos, seg->start_pos + seg->index_length - 1);
            avio_printf(out, "/>\n");
        }
                                ...
                                int final)
{
    ...
    if (!final && c->ldash && as->max_frag_duration && !(c->profile & MPD_PROFILE_DVB))
        avio_printf(out, "\t\t\t<Resync dT=\"%lld\" type=\"0\"/>\n", as->max_frag_duration);
    if (as->trick_idx >= 0)
    ...
    for (i = 0; i < s->nb_streams; i++)
    {
        ...
        {
            avio_printf(out, "\t\t\t\t<ProducerReferenceTime id=\"%d\" inband=\"true\" type=\"%s\" wallClockTime=\"%s\" presentationTime=\"%lld\">\n",
        ...
        if (!final && c->ldash && os->gop_size && os->frag_type != FRAG_TYPE_NONE && !(c->profile & MPD_PROFILE_DVB) &&
            ...
            avio_printf(out, "\t\t\t\t<Resync dT=\"%lld\" type=\"1\"/>\n", os->gop_size);
...
static int write_manifest(AVFormatContext *s, int final)
{
    ...
    else
    {
        ...
        avio_printf(out, "\tminimumUpdatePeriod=\"PT%lldS\"\n", update_period);
        if (!c->ldash)
            avio_printf(out, "\tsuggestedPresentationDelay=\"PT%lldS\"\n", c->last_duration / AV_TIME_BASE);
    ...
    {
        avio_printf(out, "\t\t<Latency target=\"%lld\"", c->target_latency / 1000);
        if (s->nb_streams > 1)
...
static int dash_init(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++)
    {
        ...
        {
            av_log(s, AV_LOG_ERROR, "Segment duration %lld is outside the allowed range for DVB-DASH profile\n", os->seg_duration);
            return AVERROR(EINVAL);
        }
        ...
        {
            av_log(s, AV_LOG_ERROR, "Fragment duration %lld is longer than Segment duration %lld\n", os->frag_duration, os->seg_duration);
            return AVERROR(EINVAL);
        }
