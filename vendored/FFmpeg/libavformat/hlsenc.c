...
static int replace_int_data_in_filename(char **s, const char *filename, char placeholder, int64_t number)
{
    ...
    for (;;)
    {
        ...
        else if (c == '%' && (av_isdigit(*(p + 1)) || *(p + 1) == placeholder))
        {
            ...
            {
                av_bprintf(&buf, "%0*lld", (number < 0) ? nd : nd++, number);
                p += (addchar_count + 1);
                addchar_count = 0;
                found_count++;
            }
                              ...
                              int64_t size)
{
    ...
    if ((find_segment_by_filename(vs->segments, filename) || find_segment_by_filename(vs->old_segments, filename)) && !byterange_mode)
    {
        ...
        if (av_strstart(line, "#EXT-X-MEDIA-SEQUENCE:", &ptr))
        {
            ...
            if (tmp_sequence < vs->sequence)
                av_log(hls, AV_LOG_VERBOSE,
                       ...
                       ""
                       "than specified start sequence number: %lld < %lld, "
                       "omitting\n",
                       tmp_sequence, hls->start_sequence);
            else
            {
                av_log(hls, AV_LOG_DEBUG, "Found playlist sequence number: %lld\n", tmp_sequence);
            ...
            if (hls->max_seg_size > 0)
            {
                    ...
                    (outer_st->codecpar->bit_rate > hls->max_seg_size))
                {
                    av_log(s, AV_LOG_WARNING, "Your video bitrate is bigger than hls_segment_size, "
                                              "(%lld > %lld), the result maybe not be what you want.",
                           outer_st->codecpar->bit_rate, hls->max_seg_size);
                }
            }
...
static int hls_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (is_ref_pkt)
    {
        ...
        else
        {
            ...
            {
                av_log(s, AV_LOG_WARNING, "Stream %d packet with pts %lld has duration 0. The segment duration may not be precise.\n",
    ...
    if (vs->packets_written && can_split && av_compare_ts(pkt->pts - vs->start_pts, st->time_base, end_pts, AV_TIME_BASE_Q) >= 0)
    {
        int64_t new_start_pos;
        int byterange_mode = (hls->flags & HLS_SINGLE_FILE) || (hls->max_seg_size > 0);
        double cur_duration;
...
static int hls_init(AVFormatContext *s)
{
        ...
        (hls->start_sequence_source_type == HLS_START_SEQUENCE_AS_FORMATTED_DATETIME))
    {
        ...
        av_log(hls, AV_LOG_DEBUG, "start_number evaluated to %lld\n", hls->start_sequence);
    }
