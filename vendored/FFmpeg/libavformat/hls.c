                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    if (prev_segments)
    {
        if (pls->start_seq_no > prev_start_seq_no && c->first_timestamp != AV_NOPTS_VALUE)
        {
            ...
            av_log(c->ctx, AV_LOG_DEBUG, "Media sequence change (%lld -> %lld)"
                                         " reflected in first_timestamp: %lld -> %lld\n",
                   prev_start_seq_no, pls->start_seq_no,
                   prev_timestamp, c->first_timestamp);
        }
        else if (pls->start_seq_no < prev_start_seq_no)
        {
            av_log(c->ctx, AV_LOG_WARNING, "Media sequence changed unexpectedly: %lld -> %lld\n",
                   prev_start_seq_no, pls->start_seq_no);
        }
    ...
    for (meta = *extra_meta; meta; meta = meta->next)
    {
        if (!strcmp(meta->tag, "PRIV"))
        {
            ...
            if (priv->datasize == 8 && !av_strncasecmp(priv->owner, id3_priv_owner_ts, 44))
            {
                ...
                av_log(s, AV_LOG_DEBUG, "HLS ID3 audio timestamp %lld\n", ts);
                if ((ts & ~((1ULL << 33) - 1)) == 0)
                    *dts = ts;
                else
                    av_log(s, AV_LOG_ERROR, "Invalid HLS ID3 audio timestamp %lld\n", ts);
            }
                          ...
                          int buf_size, int *len)
{
    ...
    while (1)
    {
        ...
        if (ff_id3v2_match(buf, ID3v2_DEFAULT_MAGIC))
        {
            ...
            if (taglen > maxsize)
            {
                av_log(pls->parent, AV_LOG_ERROR, "Too large HLS ID3 tag (%d > %lld bytes)\n",
                       taglen, maxsize);
                break;
            }
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg, AVIOContext **in)
{
    ...
    av_log(pls->parent, AV_LOG_VERBOSE, "HLS request for url '%s', offset %lld, playlist %d\n",
    ...
    if (ret == 0 && !is_http && seg->url_offset)
    {
        ...
        if (seekret < 0)
        {
            av_log(pls->parent, AV_LOG_ERROR, "Unable to seek to offset %lld of HLS segment '%s'\n", seg->url_offset, seg->url);
            ret = seekret;
    ...
    reload:
        ...
        if (v->cur_seq_no < v->start_seq_no)
        {
            av_log(v->parent, AV_LOG_WARNING,
                   "skipping %lld segments ahead, expired from playlists\n",
        ...
        if (ret < 0)
        {
            if (ff_check_interrupt(c->interrupt_callback))
                return AVERROR_EXIT;
            av_log(v->parent, AV_LOG_WARNING, "Failed to open segment %lld of playlist %d\n",
                   v->cur_seq_no,
                   v->index);
            if (segment_retries >= c->seg_max_retry)
            {
                av_log(v->parent, AV_LOG_WARNING, "Segment %lld of playlist %d failed too many times, skipping\n",
                       v->cur_seq_no,
                       v->index);
                v->cur_seq_no++;
                segment_retries = 0;
...
static int64_t select_cur_seq_no(HLSContext *c, struct playlist *pls)
{
    ...
    if (!pls->finished)
    {
        ...
        if (pls->time_offset_flag && c->prefer_x_start)
        {
            ...
            int64_t cur_timestamp = c->cur_timestamp == AV_NOPTS_VALUE ? 0 : c->cur_timestamp;
...
static int hls_read_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < c->n_playlists; i++)
    {
            ...
            pls->audio_setup_info.codec_id != AV_CODEC_ID_NONE)
        {
            ...
            in_fmt = av_find_input_format(pls->audio_setup_info.codec_id == AV_CODEC_ID_AAC ? "aac" : pls->audio_setup_info.codec_id == AV_CODEC_ID_AC3 ? "ac3"
                                                                                                                                                        : "eac3");
        }
        else
        {
...
static int recheck_discard_flags(AVFormatContext *s, int first)
{
    ...
    for (i = 0; i < c->n_playlists; i++)
    {
        ...
        if (cur_needed && !pls->needed)
        {
            ...
            av_log(s, AV_LOG_INFO, "Now receiving playlist %d, segment %lld\n", i, pls->cur_seq_no);
        }
...
static const AVOption hls_options[] = {
    ...
    {"prefer_x_start", "prefer to use #EXT-X-START if it's in playlist instead of live_start_index", OFFSET(prefer_x_start), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS},
    {"allowed_extensions", "List of file extensions that hls is allowed to access", OFFSET(allowed_extensions), AV_OPT_TYPE_STRING, {.str = "3gp,aac,avi,ac3,eac3,flac,mkv,m3u8,m4a,m4s,m4v,mpg,mov,mp2,mp3,mp4,mpeg,mpegts,ogg,ogv,oga,ts,vob,wav"}, INT_MIN, INT_MAX, FLAGS},
    {"max_reload", "Maximum number of times a insufficient list is attempted to be reloaded", OFFSET(max_reload), AV_OPT_TYPE_INT, {.i64 = 3}, 0, INT_MAX, FLAGS},
    {"m3u8_hold_counters", "The maximum number of times to load m3u8 when it refreshes without new segments", OFFSET(m3u8_hold_counters), AV_OPT_TYPE_INT, {.i64 = 1000}, 0, INT_MAX, FLAGS},
    {"http_persistent", "Use persistent HTTP connections", OFFSET(http_persistent), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, FLAGS},
    {"http_multiple", "Use multiple HTTP connections for fetching segments", OFFSET(http_multiple), AV_OPT_TYPE_BOOL, {.i64 = -1}, -1, 1, FLAGS},
    {"http_seekable", "Use HTTP partial requests, 0 = disable, 1 = enable, -1 = auto", OFFSET(http_seekable), AV_OPT_TYPE_BOOL, {.i64 = -1}, -1, 1, FLAGS},
    {"seg_format_options", "Set options for segment demuxer", OFFSET(seg_format_opts), AV_OPT_TYPE_DICT, {.str = NULL}, 0, 0, FLAGS},
