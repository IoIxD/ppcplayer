                              ...
                              unsigned int codec_data_size, const uint8_t *mime)
{
    ...
    // Duplicate tags
    if (st->codecpar->codec_type != AVMEDIA_TYPE_UNKNOWN && st->codecpar->codec_type != AVMEDIA_TYPE_DATA)
        return AVERROR_INVALIDDATA;
    ...
    do
    {
        ...
        else if ((avio_size(pb) - avio_tell(pb)) / 14 < n_pkts)
        {
            av_log(s, AV_LOG_ERROR,
                   ...
                   "exceeds filesize (%lld at %lld = %lld)\n",
            ...
            if (state == MKBETAG('I', 'N', 'D', 'X'))
            {
                ...
                if (len == 20 && expected_len <= INT_MAX)
                    ...
                    av_log(s, AV_LOG_WARNING,
                           "Index size %d (%d pkts) is wrong, should be %lld.\n",
    ...
    for (;;)
    {
        ...
        else
        {
            if (rm->old_format)
            {
                ...
                ast = st->priv_data;
                timestamp = AV_NOPTS_VALUE;
                len = !ast->audio_framesize ? RAW_PACKET_SIZE : ast->coded_framesize * (int64_t)ast->sub_packet_h / 2;
                if (len > INT_MAX)
                    return AVERROR_INVALIDDATA;
                flags = (seq++ == 1) ? 2 : 0;
                pos = avio_tell(s->pb);
                           ...
                           int64_t *ppos, int64_t pos_limit)
{
    ...
    for (;;)
    {
        ...
        if ((flags & 2) && (seq & 0x7F) == 1)
        {
            av_log(s, AV_LOG_TRACE, "%d %d-%d %lld %d\n",
