...
#define IS_WEBM(mkv) (CONFIG_WEBM_MUXER && CONFIG_MATROSKA_MUXER ? ((mkv)->mode == MODE_WEBM) : CONFIG_WEBM_MUXER)
        ...
        if (qt_id)
        {
            if (!par->codec_tag)
                par->codec_tag = ff_codec_get_tag(ff_codec_movvideo_tags,
                                                  par->codec_id);
            if (ff_codec_get_id(ff_codec_movvideo_tags, par->codec_tag) == par->codec_id && (!extradata_size || ff_codec_get_id(ff_codec_movvideo_tags, AV_RL32(extradata + 4)) != par->codec_id))
            {
                                           ...
                                           mkv_track *track, const AVStream *st)
{
    ...
    if (dovi->dv_profile <= 10)
    {
        ...
        uint64_t expected_size = (2 + 1 + (sizeof(DVCC_DVVC_BLOCK_TYPE_NAME) - 1)) + (2 + 1 + 4) + (2 + 1 + ISOM_DVCC_DVVC_SIZE);
                           ...
                           int is_default)
{
    ...
    else
    {
        ...
        if (st->disposition & (AV_DISPOSITION_ORIGINAL | AV_DISPOSITION_DUB) &&
            (st->disposition & (AV_DISPOSITION_ORIGINAL | AV_DISPOSITION_DUB)) != (AV_DISPOSITION_ORIGINAL | AV_DISPOSITION_DUB))
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (audio_frame_samples)
            mkv_write_default_duration(track, pb, (AVRational){audio_frame_samples, par->sample_rate});
        if (!native_id)
...
static int mkv_write_chapters(AVFormatContext *s)
{
    ...
    for (unsigned i = 0; i < s->nb_chapters; i++)
    {
        ...
        {
            av_log(s, AV_LOG_ERROR,
                   "Invalid chapter start (%lld) or end (%lld).\n",
...
static int64_t get_metadata_duration(AVFormatContext *s)
{
    ...
    {
        av_log(s, AV_LOG_DEBUG, "get_metadata_duration found duration in context metadata: %lld\n", us);
        return us;
    }
    ...
    av_log(s, AV_LOG_DEBUG, "get_metadata_duration returned: %lld\n", max);
    return max;
}
                           ...
                           int force_blockgroup, int64_t relative_packet_pos)
{
    ...
    av_log(logctx, AV_LOG_DEBUG,
           "Writing block of size %d with pts %lld, dts %lld, "
           "duration %lld at relative offset %lld in cluster "
           "at offset %lld. TrackNumber %u, keyframe %d\n",
...
const FFOutputFormat ff_matroska_muxer = {
    ...
    .p.audio_codec = CONFIG_LIBVORBIS_ENCODER ? AV_CODEC_ID_VORBIS : AV_CODEC_ID_AC3,
