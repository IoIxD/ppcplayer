...
void ff_configure_buffers_for_index(AVFormatContext *s, int64_t time_tolerance)
{
    ...
    {
        av_log(s, AV_LOG_VERBOSE, "Reconfiguring buffers to size %lld\n", pos_delta);
                                                     ...
                                                     int, int64_t *, int64_t))
{
    ...
    while (pos_min < pos_limit)
    {
        ...
        if (pos == pos_max)
            ...
            no_change = 0;
        av_log(s, AV_LOG_TRACE, "%lld %lld %lld / %s %s %s"
                                " target:%s limit:%lld start:%lld noc:%d\n",
               pos_min, pos, pos_max,
    ...
    av_log(s, AV_LOG_TRACE, "pos=0x%lld %s<=%s<=%s\n",
                       ...
                       int64_t ts, int64_t max_ts, int flags)
{
    ...
    if (ffifmt(s->iformat)->read_seek2)
    {
        ...
        ret = ffifmt(s->iformat)->read_seek2(s, stream_index, min_ts, ts, max_ts, flags);
