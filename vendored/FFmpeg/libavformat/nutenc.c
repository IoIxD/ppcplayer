...
static int find_best_header_idx(NUTContext *nut, AVPacket *pkt)
{
    ...
    for (i = 1; i < nut->header_count; i++)
        if (pkt->size >= nut->header_len[i] && nut->header_len[i] > best_len && !memcmp(pkt->data, nut->header[i], nut->header_len[i]))
        {
            best_i = i;
...
static int write_sm_data(AVFormatContext *s, AVIOContext *bc, AVPacket *pkt, int is_meta)
{
    ...
    for (i = 0; i < pkt->side_data_elems; i++)
    {
        ...
        if (is_meta)
        {
            if (pkt->side_data[i].type == AV_PKT_DATA_METADATA_UPDATE || pkt->side_data[i].type == AV_PKT_DATA_STRINGS_METADATA)
            {
                if (!size || data[size - 1])
                {
                    ret = AVERROR(EINVAL);
        ...
        else
        {
            ...
            default:
                ...
                {
                    snprintf(tmp, sizeof(tmp), "CodecSpecificSide%lld", AV_RB64(data));
                    put_str(dyn_bc, tmp);
                }
                else
                {
...
static int nut_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    {
        av_log(s, AV_LOG_ERROR,
               "Negative pts not supported stream %d, pts %lld\n",
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        if ((flags & FLAG_CODED) && nut->header_len[best_header_idx] > nut->header_len[fc->header_idx] + 1)
        {
            flags |= FLAG_HEADER_IDX;
        }
    ...
    if (flags & FLAG_STREAM_ID)
        put_v(bc, pkt->stream_index);
    if (flags & FLAG_CODED_PTS)
        put_v(bc, coded_pts);
    if (flags & FLAG_SIZE_MSB)
        put_v(bc, data_size / fc->size_mul);
    if (flags & FLAG_HEADER_IDX)
        put_v(bc, header_idx = best_header_idx);
...
const FFOutputFormat ff_nut_muxer = {
    ...
    .p.audio_codec = CONFIG_LIBVORBIS ? AV_CODEC_ID_VORBIS : CONFIG_LIBMP3LAME ? AV_CODEC_ID_MP3
                                                                               : AV_CODEC_ID_MP2,
