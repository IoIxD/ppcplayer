...
static const AVOption options[] = {
    ...
    {"max_size", "Max file size supported (in bytes). Default is 5MB. Set to 0 for no limit (not recommended)", OFFSET(max_size), AV_OPT_TYPE_INT, {.i64 = FF_MODPLUG_DEF_FILE_SIZE}, 0, FF_MODPLUG_MAX_FILE_SIZE, D},
...
static int modplug_load_metadata(AVFormatContext *s)
{
    ...
    if (name && *name)
        av_dict_set(&s->metadata, "name", name, 0);
    if (msg && *msg)
        av_dict_set(&s->metadata, "message", msg, 0);
...
static int modplug_read_header(AVFormatContext *s)
{
    ...
    {
        sz = modplug->max_size;
        av_log(s, AV_LOG_WARNING, "Max file size reach%s, allocating %lldB "
    ...
    if (modplug->reverb_depth)
        settings.mReverbDepth = modplug->reverb_depth;
    if (modplug->reverb_delay)
        settings.mReverbDelay = modplug->reverb_delay;
    if (modplug->bass_amount)
        settings.mBassAmount = modplug->bass_amount;
    if (modplug->bass_range)
        settings.mBassRange = modplug->bass_range;
    if (modplug->surround_depth)
        settings.mSurroundDepth = modplug->surround_depth;
    if (modplug->surround_delay)
        settings.mSurroundDelay = modplug->surround_delay;
