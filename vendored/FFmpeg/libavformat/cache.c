...
static int64_t cache_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    if (whence == AVSEEK_SIZE)
    {
        ...
        if (pos <= 0)
        {
            ...
            if (ffurl_seek(c->inner, c->inner_pos, SEEK_SET) < 0)
                av_log(h, AV_LOG_ERROR, "Inner protocol failed to seekback end : %lld\n", pos);
        }
        if (pos > 0)
            c->is_true_eof = 1;
    ...
    if ((whence == SEEK_SET && pos >= c->logical_pos ||
         whence == SEEK_END && pos <= 0) &&
        ret < 0)
    {
        if ((whence == SEEK_SET && c->read_ahead_limit >= pos - c->logical_pos) || c->read_ahead_limit < 0)
        {
            uint8_t tmp[32768];
...
static int cache_close(URLContext *h)
{
    ...
    av_log(h, AV_LOG_INFO, "Statistics, cache hits:%lld cache misses:%lld\n",
