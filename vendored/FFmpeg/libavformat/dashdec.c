                                         ...
                                         xmlNodePtr adaptionset_supplementalproperty_node)
{
    ...
    c->max_url_size = aligned(c->max_url_size + (rep->id ? strlen(rep->id) : 0) + (rep_bandwidth_val ? strlen(rep_bandwidth_val) : 0));
    ...
    if (representation_segmenttemplate_node || fragment_template_node || period_segmenttemplate_node)
    {
        ...
        if (val)
        {
            ...
            av_log(s, AV_LOG_TRACE, "rep->presentation_timeoffset = [%lld]\n", rep->presentation_timeoffset);
            xmlFree(val);
        }
        ...
        if (val)
        {
            ...
            av_log(s, AV_LOG_TRACE, "rep->fragment_duration = [%lld]\n", rep->fragment_duration);
            xmlFree(val);
        }
        ...
        if (val)
        {
            ...
            av_log(s, AV_LOG_TRACE, "rep->fragment_timescale = [%lld]\n", rep->fragment_timescale);
            xmlFree(val);
        }
        ...
        if (val)
        {
            ...
            av_log(s, AV_LOG_TRACE, "rep->first_seq_no = [%lld]\n", rep->first_seq_no);
            xmlFree(val);
        }
...
static int64_t calc_cur_seg_no(AVFormatContext *s, struct representation *pls)
{
    ...
    if (c->is_live)
    {
        ...
        {
            av_log(s, AV_LOG_TRACE, "in fragment_duration mode fragment_timescale = %lld, presentation_timeoffset = %lld\n", pls->fragment_timescale, pls->presentation_timeoffset);
...
static struct fragment *get_current_fragment(struct representation *pls)
{
    ...
    if (c->is_live)
    {
        ...
        {
            av_log(pls->parent, AV_LOG_VERBOSE, "old fragment: cur[%lld] min[%lld] max[%lld]\n", (int64_t)pls->cur_seq_no, min_seq_no, max_seq_no);
            pls->cur_seq_no = calc_cur_seg_no(pls->parent, pls);
        }
        else if (pls->cur_seq_no > max_seq_no)
        {
            av_log(pls->parent, AV_LOG_VERBOSE, "new fragment: min[%lld] max[%lld]\n", min_seq_no, max_seq_no);
        }
        seg = av_mallocz(sizeof(struct fragment));
        if (!seg)
        {
...
static int open_input(DASHContext *c, struct representation *pls, struct fragment *seg)
{
    ...
    av_log(pls->parent, AV_LOG_VERBOSE, "DASH request for url '%s', offset %lld\n",
           url, seg->url_offset);
...
static int dash_seek(AVFormatContext *s, struct representation *pls, int64_t seek_pos_msec, int flags, int dry_run)
{
    ...
    av_log(pls->parent, AV_LOG_VERBOSE, "DASH seek pos[%lldms] %s\n",
    ...
    set_seq_num:
        ...
        av_log(pls->parent, AV_LOG_VERBOSE, "dash_seek with SegmentTimeline end cur_seq_no[%lld].\n",
...
static const AVOption dash_options[] = {
    {"allowed_extensions", "List of file extensions that dash is allowed to access", OFFSET(allowed_extensions), AV_OPT_TYPE_STRING, {.str = "aac,m4a,m4s,m4v,mov,mp4,webm,ts"}, INT_MIN, INT_MAX, FLAGS},
