...
int ff_iamf_add_audio_element(IAMFContext *iamf, const AVStreamGroup *stg, void *log_ctx)
{
    ...
    for (int i = 0; i < iamf->nb_audio_elements; i++)
    {
        ...
        {
            av_log(log_ctx, AV_LOG_ERROR, "Duplicated Audio Element id %lld\n", stg->id);
            return AVERROR(EINVAL);
        }
    }
...
int ff_iamf_add_mix_presentation(IAMFContext *iamf, const AVStreamGroup *stg, void *log_ctx)
{
    ...
    for (int i = 0; i < iamf->nb_mix_presentations; i++)
    {
        ...
        {
            av_log(log_ctx, AV_LOG_ERROR, "Duplicate Mix Presentation id %lld\n", stg->id);
            return AVERROR(EINVAL);
        }
    }
                              ...
                              unsigned audio_substream_id, const AVPacket *pkt)
{
    ...
    int dyn_size, type = audio_substream_id <= 17 ? audio_substream_id + IAMF_OBU_IA_AUDIO_FRAME_ID0 : IAMF_OBU_IA_AUDIO_FRAME;
    int ret;
