...
static void add_keyframes_index(AVFormatContext *s)
{
    ...
    if (ffstream(stream)->nb_index_entries == 0)
    {
        ...
        {
            av_log(s, AV_LOG_TRACE, "keyframe filepositions = %lld times = %lld\n",
...
static AVStream *create_stream(AVFormatContext *s, int codec_type)
{
    ...
    if (s->nb_streams >= 3 || (s->nb_streams == 2 && s->streams[0]->codecpar->codec_type != AVMEDIA_TYPE_SUBTITLE && s->streams[1]->codecpar->codec_type != AVMEDIA_TYPE_SUBTITLE && s->streams[0]->codecpar->codec_type != AVMEDIA_TYPE_DATA && s->streams[1]->codecpar->codec_type != AVMEDIA_TYPE_DATA))
                            ...
                            int64_t max_pos, int depth)
{
    ...
    if (key)
    {
        ...
        else if (amf_type == AMF_DATA_TYPE_DATE && isfinite(date.milliseconds) && date.milliseconds > INT64_MIN / 1000 && date.milliseconds < INT64_MAX / 1000)
        {
    ...
    for (i = 0; !avio_feof(s->pb); i++)
    {
        ...
        if (i >= 8 && pos)
        {
            ...
            if (d[0] == 'F' &&
                ...
                d[3] < 5 && d[5] == 0)
            {
                av_log(s, AV_LOG_WARNING, "Concatenated FLV detected, might fail to demux, decode and seek %lld\n", flv->last_ts);
...
retry:
    ...
    av_log(s, AV_LOG_TRACE, "type:%d, size:%d, last:%d, dts:%lld pos:%lld\n", type, size, last, dts, avio_tell(s->pb));
    if (avio_feof(s->pb))
        return AVERROR_EOF;
...
leave:
    ...
    if (!flv->trust_datasize)
    {
        if (last != orig_size + 11 && last != orig_size + 10 &&
            ...
            !flv->broken_sizes)
        {
            av_log(s, AV_LOG_ERROR, "Packet mismatch %d %d %lld\n", last, orig_size + 11, flv->sum_flv_tag_size);
