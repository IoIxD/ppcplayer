...
retry:
    for (i = 0; i < ctx->nb_streams; i++)
    {
        ...
        if (s->packet_size > avail_data && !flush && st->codecpar->codec_type != AVMEDIA_TYPE_SUBTITLE)
            return 0;
        if (avail_data == 0)
            continue;
        ...
        if (next_pkt && next_pkt->dts - scr > max_delay && !ignore_delay)
            continue;
        if (stream->predecode_packet && stream->predecode_packet->size > stream->buffer_index)
            rel_space += 1 << 28;
...
static int mpeg_mux_write_packet(AVFormatContext *ctx, AVPacket *pkt)
{
    ...
    if (s->last_scr == AV_NOPTS_VALUE)
    {
        ...
        preload = av_rescale(s->preload, 90000, AV_TIME_BASE);
        av_log(ctx, AV_LOG_DEBUG, "First SCR: %lld First DTS: %lld\n", s->last_scr, dts + preload);
    }
