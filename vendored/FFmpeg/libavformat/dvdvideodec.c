...
static int dvdvideo_menu_open(AVFormatContext *s, DVDVideoPlaybackState *state)
{
    ...
    {
        av_log(s, AV_LOG_ERROR, !c->opt_menu_vts ? "Unable to open main menu VOB (VIDEO_TS.VOB)\n" : "Unable to open menu VOBs for VTS %d\n", c->opt_menu_vts);
                                       ...
                                       void (*flush_cb)(AVFormatContext *s))
{
    ...
    if (blocks_read != 1)
    {
        av_log(s, AV_LOG_ERROR, "Unable to read VOB block: offset=%d blocks_read=%lld\n",
    ...
    if (!state->vobu_remaining)
    {
        ...
        state->vobu_next = dsi.vobu_sri.next_vobu == SRI_END_OF_CELL ? SRI_END_OF_CELL : dsi.dsi_gi.nv_pck_lbn + (dsi.vobu_sri.next_vobu & 0x7FFFFFFF);
        state->sector_offset++;
        ...
        av_log(s, AV_LOG_DEBUG, "NAV packet: sector=%d "
                                "vobu_s_ptm=%d vobu_e_ptm=%d ts_offset=%lld\n",
    ...
    for (int i = 0; i < DVDVIDEO_MAX_PS_SEARCH_BLOCKS; i++)
    {
        ...
        case DVDNAV_NAV_PACKET:
            ...
            av_log(s, AV_LOG_DEBUG,
                   ...
                   "scr=%d lbn=%d vobu_duration=%d nav_pts=%lld\n",
                   e_pci->pci_gi.vobu_s_ptm, e_pci->pci_gi.vobu_e_ptm,
...
static int dvdvideo_video_stream_setup(AVFormatContext *s)
{
    ...
    if (c->opt_menu)
        video_attr = !c->opt_menu_vts ? c->vmg_ifo->vmgi_mat->vmgm_video_attr : c->vts_ifo->vtsi_mat->vtsm_video_attr;
    else
...
static int dvdvideo_audio_stream_add_all(AVFormatContext *s)
{
    ...
    if (c->opt_menu)
        nb_streams = !c->opt_menu_vts ? c->vmg_ifo->vmgi_mat->nr_of_vmgm_audio_streams : c->vts_ifo->vtsi_mat->nr_of_vtsm_audio_streams;
    else
    ...
    for (int i = 0; i < nb_streams; i++)
    {
        ...
        if (c->opt_menu)
            audio_attr = !c->opt_menu_vts ? c->vmg_ifo->vmgi_mat->vmgm_audio_attr : c->vts_ifo->vtsi_mat->vtsm_audio_attr;
        else
...
static int dvdvideo_subp_stream_add_all(AVFormatContext *s)
{
    ...
    if (c->opt_menu)
        nb_streams = !c->opt_menu_vts ? c->vmg_ifo->vmgi_mat->nr_of_vmgm_subp_streams : c->vts_ifo->vtsi_mat->nr_of_vtsm_subp_streams;
    else
    ...
    for (int i = 0; i < nb_streams; i++)
    {
        ...
        if (c->opt_menu)
        {
            ...
            subp_attr = !c->opt_menu_vts ? c->vmg_ifo->vmgi_mat->vmgm_subp_attr : c->vts_ifo->vtsi_mat->vtsm_subp_attr;
        }
        else
        {
            video_attr = c->vts_ifo->vtsi_mat->vts_video_attr;
...
static int dvdvideo_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt->pts != AV_NOPTS_VALUE && pkt->dts != AV_NOPTS_VALUE)
    {
        ...
        if (pkt->pts < 0)
        {
            av_log(s, AV_LOG_VERBOSE, "Discarding packet with negative PTS (st=%d pts=%lld), "
    ...
    {
        av_log(s, AV_LOG_WARNING, "Unset PTS or DTS @ st=%d pts=%lld dts=%lld\n",
               pkt->stream_index, pkt->pts, pkt->dts);
    }

    av_log(s, AV_LOG_TRACE, "st=%d pts=%lld dts=%lld "
                            "ts_offset=%lld first_pts=%lld\n",
           pkt->stream_index, pkt->pts, pkt->dts,
