    ...
    if (rf64 || bw64)
    {
        ...
        if (data_size < 0 || sample_count < 0)
        {
            av_log(s, AV_LOG_ERROR, "negative data_size and/or sample_count in "
                                    "ds64: data_size = %lld, sample_count = %lld\n",
...
break_loop:
    ...
    {
        av_log(s, AV_LOG_WARNING, "Data size %lld is too large\n", data_size);
        data_size = 0;
    }

    if (st->codecpar->bit_rate > 0 && data_size > 0 && st->codecpar->sample_rate > 0 && sample_count > 0 && st->codecpar->ch_layout.nb_channels > 1 && sample_count % st->codecpar->ch_layout.nb_channels == 0)
    {
        if (fabs(8.0 * data_size * st->codecpar->ch_layout.nb_channels * st->codecpar->sample_rate /
                     sample_count / st->codecpar->bit_rate -
                 1.0) < 0.3)
    ...
    {
        av_log(s, AV_LOG_WARNING, "ignoring wrong sample_count %lld\n", sample_count);
        sample_count = 0;
    }
    ...
    if (!sample_count || av_get_exact_bits_per_sample(st->codecpar->codec_id) > 0)
        if (st->codecpar->ch_layout.nb_channels && data_size && av_get_bits_per_sample(st->codecpar->codec_id) && wav->data_end <= avio_size(pb))
...
static int wav_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    smv_retry:
        ...
        if (audio_dts != AV_NOPTS_VALUE && video_dts != AV_NOPTS_VALUE)
        {
            ...
            wav->smv_last_stream = wav->smv_given_first ? av_compare_ts(video_dts, vst->time_base,
                                                                        audio_dts, st->time_base) > 0
                                                        : 0;
