...
static AVIOContext *wtvfile_open_sector(unsigned first_sector, uint64_t length, int depth, AVFormatContext *s)
{
    ...
    {
        av_log(s, AV_LOG_WARNING, "reported file length (0x%lld) exceeds number of available sectors (0x%lld)\n", length, (int64_t)wf->nb_sectors << wf->sector_bits);
        length = (int64_t)wf->nb_sectors << wf->sector_bits;
    }
    wf->length = length;
...
static void parse_mpeg1waveformatex(AVStream *st)
{
    ...
    {
    case 0x0001:
        st->codecpar->codec_id = AV_CODEC_ID_MP1;
        break;
    case 0x0002:
                                  ...
                                  ff_asf_guid formattype, uint64_t size)
{
    ...
    av_log(s, AV_LOG_WARNING, "unknown media type, mediatype:" FF_PRI_GUID ", subtype:" FF_PRI_GUID ", formattype:" FF_PRI_GUID "\n",
