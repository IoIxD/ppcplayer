...
static int mxf_utf16_local_tag_length(const char *utf8_str)
{
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "utf16 local tag size %lld invalid (too large), ignoring\n", size);
        return 0;
    }
...
static void mxf_write_identification(AVFormatContext *s)
{
    ...
    const char *product = product_entry ? product_entry->value : !IS_OPATOM(s) ? "OP1a Muxer"
                                                                               : "OPAtom Muxer";
    const char *platform = s->flags & AVFMT_FLAG_BITEXACT ? "Lavf" : PLATFORM_IDENT;
    const char *version = version_entry ? version_entry->value : s->flags & AVFMT_FLAG_BITEXACT ? "0.0.0"
...
static int64_t mxf_write_cdci_common(AVFormatContext *s, AVStream *st, const UID key)
{
    ...
    case 576:
        f1 = 23;
        f2 = st->codecpar->codec_id == AV_CODEC_ID_DVVIDEO ? 335 : 336;
        break;
    case 608:
        f1 = 7;
        f2 = 320;
    ...
    case 480:
        f1 = 20;
        f2 = st->codecpar->codec_id == AV_CODEC_ID_DVVIDEO ? 285 : 283;
        break;
    case 512:
        f1 = 7;
        f2 = 270;
...
static int mxf_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    {
        if ((err = av_reallocp_array(&mxf->index_entries, mxf->edit_units_count + EDIT_UNITS_PER_BODY, sizeof(*mxf->index_entries))) < 0)
        {
            mxf->edit_units_count = 0;
...
static int mxf_interleave_get_packet(AVFormatContext *s, AVPacket *out, int flush)
{
    ...
    if (stream_count && (s->nb_streams == stream_count || flush))
    {
        ...
        av_log(s, AV_LOG_TRACE, "out st:%d dts:%lld\n", out->stream_index, out->dts);
        return 1;
    }
    else
    {
...
#define MXF_COMMON_OPTIONS                                                                                                                                                                              \
    {"signal_standard", "Force/set Signal Standard", offsetof(MXFContext, signal_standard), AV_OPT_TYPE_INT, {.i64 = -1}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},                \
        {"bt601", "ITU-R BT.601 and BT.656, also SMPTE 125M (525 and 625 line interlaced)", 0, AV_OPT_TYPE_CONST, {.i64 = 1}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},            \
        {"bt1358", "ITU-R BT.1358 and ITU-R BT.799-3, also SMPTE 293M (525 and 625 line progressive)", 0, AV_OPT_TYPE_CONST, {.i64 = 2}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"}, \
        {"smpte347m", "SMPTE 347M (540 Mbps mappings)", 0, AV_OPT_TYPE_CONST, {.i64 = 3}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},                                                \
        {"smpte274m", "SMPTE 274M (1125 line)", 0, AV_OPT_TYPE_CONST, {.i64 = 4}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},                                                        \
        {"smpte296m", "SMPTE 296M (750 line progressive)", 0, AV_OPT_TYPE_CONST, {.i64 = 5}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},                                             \
        {"smpte349m", "SMPTE 349M (1485 Mbps mappings)", 0, AV_OPT_TYPE_CONST, {.i64 = 6}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},                                               \
        {"smpte428", "SMPTE 428-1 DCDM", 0, AV_OPT_TYPE_CONST, {.i64 = 7}, -1, 7, AV_OPT_FLAG_ENCODING_PARAM, .unit = "signal_standard"},
...
static const AVOption d10_options[] = {
    {"d10_channelcount", "Force/set channelcount in generic sound essence descriptor", offsetof(MXFContext, channel_count), AV_OPT_TYPE_INT, {.i64 = -1}, -1, 8, AV_OPT_FLAG_ENCODING_PARAM},
...
static const AVOption opatom_options[] = {
    {"mxf_audio_edit_rate", "Audio edit rate for timecode", offsetof(MXFContext, audio_edit_rate), AV_OPT_TYPE_RATIONAL, {.dbl = 25}, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    MXF_COMMON_OPTIONS{"store_user_comments", "", offsetof(MXFContext, store_user_comments), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, AV_OPT_FLAG_ENCODING_PARAM},
    {NULL},
};
