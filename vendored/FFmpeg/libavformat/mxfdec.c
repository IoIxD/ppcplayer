...
static int mxf_get_eia608_packet(AVFormatContext *s, AVStream *st, AVPacket *pkt, int64_t length)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (length < 6)
        {
            av_log(s, AV_LOG_ERROR, "error reading s436m packet %lld\n", length);
            return AVERROR_INVALIDDATA;
        }
...
static int mxf_read_partition_pack(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    if (this_partition != klv_offset - mxf->run_in)
    {
        av_log(mxf->fc, AV_LOG_ERROR,
               "this_partition %lld mismatches %lld\n",
    ...
    if (op[12] == 1 && op[13] == 1)
        mxf->op = OP1a;
    else if (op[12] == 1 && op[13] == 2)
        mxf->op = OP1b;
    else if (op[12] == 1 && op[13] == 3)
        mxf->op = OP1c;
    else if (op[12] == 2 && op[13] == 1)
        mxf->op = OP2a;
    else if (op[12] == 2 && op[13] == 2)
        mxf->op = OP2b;
    else if (op[12] == 2 && op[13] == 3)
        mxf->op = OP2c;
    else if (op[12] == 3 && op[13] == 1)
        mxf->op = OP3a;
    else if (op[12] == 3 && op[13] == 2)
        mxf->op = OP3b;
    else if (op[12] == 3 && op[13] == 3)
        mxf->op = OP3c;
    else if (op[12] == 64 && op[13] == 1)
        mxf->op = OPSONYOpt;
    else if (op[12] == 0x10)
    {
...
static int mxf_read_index_table_segment(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x3F0C:
        ...
        av_log(NULL, AV_LOG_TRACE, "IndexStartPosition %lld\n", segment->index_start_position);
        break;
    case 0x3F0D:
        segment->index_duration = avio_rb64(pb);
        av_log(NULL, AV_LOG_TRACE, "IndexDuration %lld\n", segment->index_duration);
        break;
    }
    return 0;
}
...
static int mxf_get_sorted_table_segments(MXFContext *mxf, int *nb_sorted_segments, MXFIndexTableSegment ***sorted_segments)
{
    ...
    for (i = nb_segments = 0; i < mg->metadata_sets_count; i++)
    {
        ...
        if (s->edit_unit_byte_count || s->nb_index_entries)
            ...
            av_log(mxf->fc, AV_LOG_WARNING, "IndexSID %i segment at %lld missing EditUnitByteCount and IndexEntryArray\n",
...
static int mxf_edit_unit_absolute_offset(MXFContext *mxf, MXFIndexTable *index_table, int64_t edit_unit, AVRational edit_rate, int64_t *edit_unit_out, int64_t *offset_out, MXFPartition **partition_out, int nag)
{
    ...
    for (i = 0; i < index_table->nb_segments; i++)
    {
        ...
        if (edit_unit < s->index_start_position + s->index_duration)
        {
            ...
            else
            {
                ...
                {
                    av_log(mxf->fc, AV_LOG_ERROR, "IndexSID %i segment at %lld IndexEntryArray too small\n",
    ...
    if (nag)
        av_log(mxf->fc, AV_LOG_ERROR, "failed to map EditUnit %lld in IndexSID %i to an offset\n", edit_unit, index_table->index_sid);
...
static int mxf_compute_ptses_fake_index(MXFContext *mxf, MXFIndexTable *index_table)
{
    ...
    for (i = x = 0; i < index_table->nb_segments; i++)
    {
        ...
        for (j = 0; j < n; j += index_delta, x++)
        {
            ...
            {
                av_log(mxf->fc, AV_LOG_ERROR,
                       "x >= nb_ptses - IndexEntryCount %i < IndexDuration %lld?\n",
...
static int mxf_compute_index_tables(MXFContext *mxf)
{
    ...
    for (i = j = 0; j < mxf->nb_index_tables; i += mxf->index_tables[j++].nb_segments)
    {
        ...
        if (sorted_segments[i]->index_start_position)
            av_log(mxf->fc, AV_LOG_WARNING, "IndexSID %i starts at EditUnit %lld - seeking may not work as expected\n",
    ...
    for (i = 0; i < material_package->tracks_count; i++)
    {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            if (st->codecpar->codec_id == AV_CODEC_ID_PRORES)
            {
                ...
                {
                case 1:
                    st->codecpar->codec_tag = MKTAG('a', 'p', 'c', 'o');
                    break;
                case 2:
                    st->codecpar->codec_tag = MKTAG('a', 'p', 'c', 's');
                    break;
                case 3:
                    st->codecpar->codec_tag = MKTAG('a', 'p', 'c', 'n');
                    break;
                case 4:
                    st->codecpar->codec_tag = MKTAG('a', 'p', 'c', 'h');
                    break;
                case 5:
                    st->codecpar->codec_tag = MKTAG('a', 'p', '4', 'h');
                    break;
                case 6:
                    st->codecpar->codec_tag = MKTAG('a', 'p', '4', 'x');
                    break;
                }
            }
...
static int64_t mxf_set_current_edit_unit(MXFContext *mxf, AVStream *st, int64_t current_offset, int resync)
{
    ...
    av_log(mxf->fc, AV_LOG_WARNING, "edit unit sync lost on stream %d, jumping from %lld to %lld\n", st->index, edit_unit, new_edit_unit);
            ...
            IS_KLV_KEY(klv.key, mxf_avid_essence_element_key))
        {
            ...
            if (track->wrapping != FrameWrapped)
            {
                ...
                else
                {
                    ...
                    {
                        av_log(s, AV_LOG_ERROR, "bad size: %lld\n", size);
                        mxf->current_klv_data = (KLVPacket){{0}};
                        return AVERROR_INVALIDDATA;
                    }
...
static const AVOption options[] = {
    {"eia608_extract", "extract eia 608 captions from s436m track", offsetof(MXFContext, eia608_extract), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM},
    {NULL},
};
