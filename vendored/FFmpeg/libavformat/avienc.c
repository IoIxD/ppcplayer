...
static int avi_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (par->codec_type == AVMEDIA_TYPE_VIDEO && par->codec_id != AV_CODEC_ID_XSUB && au_byterate > 1000LL * au_scale)
        {
            au_byterate = 600;
            au_scale = 1;
        }
        ...
        if (par->codec_type != AVMEDIA_TYPE_DATA)
        {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!par->codec_tag && par->codec_id == AV_CODEC_ID_RAWVIDEO && par->format == AV_PIX_FMT_RGB555LE && par->bits_per_coded_sample == 15)
                    par->bits_per_coded_sample = 16;
                avist->pal_offset = avio_tell(pb) + 40;
                ff_put_bmp_header(pb, par, 0, 0, avi->flipped_raw_rgb);
                pix_fmt = avpriv_pix_fmt_find(PIX_FMT_LIST_AVI,
                                              par->bits_per_coded_sample);
                if (!par->codec_tag && par->codec_id == AV_CODEC_ID_RAWVIDEO && par->format != pix_fmt && par->format != AV_PIX_FMT_NONE)
            ...
            if (par->codec_id == AV_CODEC_ID_XSUB && (t = av_dict_get(s->streams[i]->metadata, "language", NULL, 0)))
            {
...
static int write_skip_frames(AVFormatContext *s, int stream_index, int64_t dts)
{
           ...
           dts > avist->packet_count && par->codec_id != AV_CODEC_ID_XSUB && avist->packet_count)
    {
        ...
        {
            av_log(s, AV_LOG_ERROR, "Too large number of skipped frames %lld > 60000\n", dts - avist->packet_count);
            return AVERROR(EINVAL);
        }
