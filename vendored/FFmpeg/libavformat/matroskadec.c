...
static int matroska_resync(MatroskaDemuxContext *matroska, int64_t last_pos)
{
    ...
    if (last_pos < avio_tell(pb) && avio_seek(pb, last_pos + 1, SEEK_SET) < 0)
    {
        ...
        avio_seek(pb, FFMAX(avio_tell(pb) + (pb->buffer - pb->buf_ptr), last_pos + 1), SEEK_SET);
    }
                      ...
                      EbmlSyntax *syntax, void *data)
{
    ...
    if (syntax->type != EBML_STOP)
    {
        ...
        if (matroska->num_levels > 0)
        {
            ...
            {
                level_check = 0;
                if (id != MATROSKA_ID_CLUSTER && (syntax->type == EBML_LEVEL1 || syntax->type == EBML_NEST))
                {
                                  ...
                                  MatroskaTrack *track)
{
    ...
    case MATROSKA_TRACK_ENCODING_COMP_LZO:
        do
        {
            int insize = isize;
            olen = pkt_size *= 3;
            newpktdata = av_realloc(pkt_data, pkt_size + AV_LZO_OUTPUT_PADDING + AV_INPUT_BUFFER_PADDING_SIZE);
            if (!newpktdata)
            {
...
static int get_qt_codec(MatroskaTrack *track, uint32_t *fourcc, enum AVCodecID *codec_id)
{
    ...
    codec_tags = track->type == MATROSKA_TRACK_TYPE_VIDEO ? ff_codec_movvideo_tags : ff_codec_movaudio_tags;
                                 ...
                                 AVFormatContext *s, int *extradata_offset)
{
    ...
    case AV_CODEC_ID_SIPR:
    {
        ...
        {
            if (track->audio.sub_packet_h & 1 || 2 * track->audio.frame_size != (int64_t)track->audio.sub_packet_h * track->audio.coded_framesize)
                return AVERROR_INVALIDDATA;
                           ...
                           AVFormatContext *s, int *extradata_offset)
{
    ...
    if (track->codec_delay > 0)
    {
        par->initial_padding = av_rescale_q(track->codec_delay,
                                            ...
                                            (AVRational){1, par->codec_id == AV_CODEC_ID_OPUS ? 48000 : par->sample_rate});
    }
                           ...
                           int *extradata_offset)
{
    ...
    if (track->default_duration)
    {
        ...
        if (st->avg_frame_rate.num < st->avg_frame_rate.den * 1000LL && st->avg_frame_rate.num > st->avg_frame_rate.den * 5LL)
...
static int matroska_parse_tracks(AVFormatContext *s)
{
    ...
    for (i = 0; i < matroska->tracks.nb_elem; i++)
    {
        ...
        if (track->type == MATROSKA_TRACK_TYPE_AUDIO && track->codec_id[0] != 'A' || track->type == MATROSKA_TRACK_TYPE_VIDEO && track->codec_id[0] != 'V' || track->type == MATROSKA_TRACK_TYPE_SUBTITLE && track->codec_id[0] != 'D' && track->codec_id[0] != 'S' || track->type == MATROSKA_TRACK_TYPE_METADATA && track->codec_id[0] != 'D' && track->codec_id[0] != 'S')
        {
...
static int webm_dash_manifest_cues(AVFormatContext *s, int64_t init_range)
{
    ...
    for (int i = 0; i < sti->nb_index_entries; i++)
        av_bprintf(&bprint, "%lld,", sti->index_entries[i].timestamp);
