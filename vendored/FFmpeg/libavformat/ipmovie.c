                               ...
                               AVPacket *pkt)
{
    ...
    if (s->audio_chunk_offset && s->audio_channels && s->audio_bits)
    {
        ...
        av_log(s->avf, AV_LOG_TRACE, "sending audio frame with pts %lld (%d audio frames)\n",
               pkt->pts, s->audio_frame_count);
    ...
    else if (s->frame_format)
    {
        ...
        av_log(s->avf, AV_LOG_TRACE, "sending video frame with pts %lld\n", pkt->pts);
    ...
    while ((chunk_size > 0) && (chunk_type != CHUNK_BAD))
    {
        ...
        case OPCODE_INIT_AUDIO_BUFFERS:
            ...
            av_log(s->avf, AV_LOG_TRACE, "audio: %d bits, %d Hz, %s, %s format\n",
                   ...
                   (s->audio_type == AV_CODEC_ID_INTERPLAY_DPCM) ? "Interplay audio" : "PCM");
            break;
        ...
        case OPCODE_SET_PALETTE:
            ...
            if ((first_color > 0xFF) || (last_color > 0xFF) || (last_color - first_color + 1) * 3 + 4 > opcode_size)
            {
