...
static void ts_str(char buffer[60], int64_t ts, AVRational base)
{
    ...
    snprintf(buffer, 60, "%c%lld.%06lld", ts < 0 ? '-' : ' ', FFABS(ts) / 1000000, FFABS(ts) % 1000000);
}
...
int main(int argc, char **argv)
{
    ...
    {
        if (firstback)
            avformat_seek_file(ic, -1, INT64_MIN, seekfirst, seekfirst, 0);
        else
            avformat_seek_file(ic, -1, seekfirst, seekfirst, INT64_MAX, 0);
    }
    for (i = 0;; i++)
    {
        AVPacket pkt = {0};
        ...
        if (ret >= 0)
        {
            for (j = 0; j < frame_count; j++)
            {
                ...
                if (ret >= 0)
                {
                    ...
                    printf("ret:%-10s st:%2d flags:%d dts:%s pts:%s pos:%7lld size:%6d", ret_str(ret), pkt.stream_index, pkt.flags, dts_buf, ts_buf, pkt.pos, pkt.size);
                    av_packet_unref(&pkt);
                }
                else
        ...
        if (i & 1)
            ret = avformat_seek_file(ic, stream_id, INT64_MIN, timestamp, timestamp, 0);
        else
            ret = avformat_seek_file(ic, stream_id, timestamp, timestamp, INT64_MAX, 0);
