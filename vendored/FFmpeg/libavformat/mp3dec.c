...
static int mp3_read_probe(const AVProbeData *p)
{
    ...
    if (first_frames >= 7)
    ...
    else if (max_frames > 200 && p->buf_size < 2 * max_framesizes)
        ...
        return 5;
    else if (max_frames >= 1 && p->buf_size < 10 * max_framesizes)
        return 1;
    else
        return 0;
...
#define MIDDLE_BITS(k, m, n) LAST_BITS((k) >> (m), ((n) - (m) + 1))
    ...
    if (v & XING_FLAG_TOC)
        read_xing_toc(s, mp3->header_filesize, av_rescale_q(mp3->frames, (AVRational){spf, c->sample_rate}, st->time_base));
    /* VBR quality */
    ...
    /* Encoder delays */
    v = avio_rb24(s->pb);
    if (AV_RB32(version) == MKBETAG('L', 'A', 'M', 'E') || AV_RB32(version) == MKBETAG('L', 'a', 'v', 'f') || AV_RB32(version) == MKBETAG('L', 'a', 'v', 'c'))
    {
...
static int mp3_read_header(AVFormatContext *s)
{
    ...
    {
        av_log(s, i > 0 ? AV_LOG_INFO : AV_LOG_VERBOSE, "Skipping %d bytes of junk at %lld.\n", i, off);
    ...
    for (i = 0; i < SEEK_WINDOW; i++)
    {
        ...
        for (j = 0; j < MIN_VALID; j++)
        {
            ...
            if (ret < 0)
            {
                ...
                {
                    av_log(s, AV_LOG_ERROR, "Could not seek to %lld.\n", pos);
