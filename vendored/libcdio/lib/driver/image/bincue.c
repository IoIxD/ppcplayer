...
_init_bincue(_img_private_t *p_env)
{
  ...
  if (NULL == p_env->psz_cue_name)
    return false;

  /* Read in CUE sheet. */
  if (!parse_cuefile(p_env, p_env->psz_cue_name))
    return false;
...
get_disc_last_lsn_bincue(void *p_user_data)
{
  ...
  {
    cdio_warn("image %s size (%lld) not multiple of blocksize (%d)",
...
parse_cuefile(_img_private_t *cd, const char *psz_cue_name)
{
        ...
        if (NULL != (psz_field = strtok(NULL, " \t\n\r")))
        {
          ...
          else if (0 == strcmp("MODE1/2352", psz_field))
          {
            if (cd)
            {
              ...
              this_track->datasize = CDIO_CD_FRAMESIZE;
              this_track->endsize = CDIO_CD_EDC_SIZE + CDIO_CD_M1F1_ZERO_SIZE + CDIO_CD_ECC_SIZE;
          ...
          else if (0 == strcmp("MODE2/2352", psz_field))
          {
            if (cd)
            {
              ...
              this_track->datastart = CDIO_CD_SYNC_SIZE + CDIO_CD_HEADER_SIZE + CDIO_CD_SUBHEADER_SIZE;
              this_track->datasize = CDIO_CD_FRAMESIZE;
            ...
            if (cd)
            {
              ...
              case 1:
                ...
                if (cd->gen.i_tracks > 1)
                {
                  ...
                  else if (this_track->start_lba >= prev_track->start_lba + CDIO_PREGAP_SECTORS)
                  {
  ...
  */
  ...
  _get_track_format_bincue(void *p_user_data, track_t i_track)
  {
    ...
    if (i_track > p_env->gen.i_first_track + p_env->gen.i_tracks - 1 || i_track < p_env->gen.i_first_track)
      return TRACK_FORMAT_ERROR;
  ...
  */
  ...
  _get_track_green_bincue(void *p_user_data, track_t i_track)
  {
    ...
    if (NULL == p_env ||
        (i_track < p_env->gen.i_first_track || i_track >= p_env->gen.i_tracks + p_env->gen.i_first_track))
      return false;
