								  ...
								  uint16_t i_blocksize)
{
	...
	{
		cdio_info("image %s track %d size (%lld) not a multiple"
				  " of the blocksize (%ld)",
...
_init_cdrdao(_img_private_t *env)
{
	...
	/* Read in TOC sheet. */
	if (!parse_tocfile(env, env->psz_cue_name))
		return false;
...
get_disc_last_lsn_cdrdao(void *p_user_data)
{
	...
	else
	{
		...
		else
		{
			...
			i_size = cdio_stream_stat(p_env->tocent[i_leadout - 1].data_source) - p_env->tocent[i_leadout - 1].offset;
		}
		if (i_size < 0)
		{
...
parse_tocfile(_img_private_t *cd, const char *psz_cue_name)
{
	...
	while (fgets(psz_line, MAXLINE, fp))
	{
		...
		if ((psz_keyword = strtok(psz_line, " \t\n\r")))
		{
				...
				if (NULL != (psz_field = strtok(NULL, " \t\n\r")))
				{
					...
					else if (0 == strcmp("MODE1", psz_field))
					{
						if (NULL != cd)
						{
							...
							cd->tocent[i].blocksize = CDIO_CD_FRAMESIZE_RAW;
							cd->tocent[i].datastart = CDIO_CD_SYNC_SIZE + CDIO_CD_HEADER_SIZE;
							cd->tocent[i].datasize = CDIO_CD_FRAMESIZE;
							cd->tocent[i].endsize = CDIO_CD_EDC_SIZE + CDIO_CD_M1F1_ZERO_SIZE + CDIO_CD_ECC_SIZE;
							switch (cd->disc_mode)
							{
							case CDIO_DISC_MODE_NO_INFO:
								cd->disc_mode = CDIO_DISC_MODE_CD_DATA;
					...
					else if (0 == strcmp("MODE2_FORM2", psz_field))
					{
						if (NULL != cd)
						{
							...
							cd->tocent[i].datasize = CDIO_CD_FRAMESIZE;
							cd->tocent[i].endsize = CDIO_CD_SYNC_SIZE + CDIO_CD_ECC_SIZE;
							switch (cd->disc_mode)
							{
							case CDIO_DISC_MODE_NO_INFO:
								cd->disc_mode = CDIO_DISC_MODE_CD_XA;
			...
			else if (0 == strcmp("FILE", psz_keyword) || 0 == strcmp("AUDIOFILE", psz_keyword))
			{
				if (0 <= i)
				{
					if (NULL != (psz_field = strtok(NULL, "\"\t\n\r")))
				...
				if (0 <= i)
				{
					...
					if (psz_field)
					{
						...
						if (psz_field[0] == '#')
						{
							...
							if ((LONG_MIN == offset || LONG_MAX == offset) && 0 != errno)
							{
								cdio_log(log_level,
...
bool cdio_is_tocfile(const char *psz_cue_name)
{
	...
	if (i > 0)
	{
		if ((psz_cue_name[i] == 't' && psz_cue_name[i + 1] == 'o' && psz_cue_name[i + 2] == 'c') || (psz_cue_name[i] == 'T' && psz_cue_name[i + 1] == 'O' && psz_cue_name[i + 2] == 'C'))
		{
